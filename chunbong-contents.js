...
function renderRecordStrip(item){
  ...
  const rows=[
    ...
    ['지식 가이드',documentGuideRows(item).length?documentGuideRows(item).length+'개':'없음'],
...
function renderSources(item){const rows=[...(item.sources||[])].filter(r=>!/bngts\.com|nemopix\.xyz|namu\.moe/i.test(String(r.url||''))).map(r=>({...r,label:String(r.label||'원문 자료').replace(/나무위키 계열|나무미러/g,'나무위키')})).sort((a,b)=>sourcePriority(a)-sourcePriority(b)||String(a.label||'').localeCompare(String(b.label||''),'ko'));return`<div class="archive-panel"><div class="archive-section-heading"><div><small>SOURCE ARCHIVE</small><h2>출처</h2></div><span>${rows.length}개</span></div><p class="archive-source-guide">춘봉 SOOP 방송국을 최우선으로, 춘봉TV YouTube·공개 Notion·나무위키 등 확인 가능한 원문 자료만 표시합니다. 내부 교차검증 자료는 공개 출처와 화면 문구에 노출하지 않습니다.</p><div class="archive-source-list">${rows.map(r=>`<a href="${escapeHtml(safeUrl(r.url))}" target="_blank" rel="noreferrer"><span><strong>${escapeHtml(r.label||'원문 자료')}</strong><small class="archive-source-kind is-${escapeHtml(r.kind||'reference')}">${escapeHtml(sourceKindLabel(r.kind))}</small></span><b>↗</b></a>`).join('')}</div></div>`}
...
function bindGuideImages(root=els.detail){
  root.querySelectorAll('[data-guide-image-src]').forEach(button=>button.addEventListener('click',()=>openLightbox(
    button.dataset.guideImageSrc||'',button.dataset.guideImageAlt||'가이드 이미지',
    button.dataset.guideImageSource||'',button.dataset.guideImageCaption||''
  )));
}
...
function openLightbox(src,alt,sourceUrl='',caption=''){const dialog=els.lightbox;if(!dialog)return;const image=dialog.querySelector('img'),source=dialog.querySelector('[data-archive-lightbox-source]'),cap=dialog.querySelector('[data-archive-lightbox-caption]'),sourceSafe=safeUrl(sourceUrl);image.src=safeUrl(src);image.alt=alt||'콘텐츠 이미지';cap.textContent=caption||alt||'';source.hidden=!sourceSafe;source.href=sourceSafe||'#';if(sourceSafe)source.textContent=/namu\.wiki/i.test(sourceSafe)?'나무위키 원문 보기 ↗':/notion\.(?:site|so)|app\.notion\.com/i.test(sourceSafe)?'Notion 원문 보기 ↗':/sooplive\.com/i.test(sourceSafe)?'SOOP 원문 보기 ↗':'원문 보기 ↗';if(typeof dialog.showModal==='function')dialog.showModal();else dialog.setAttribute('open','')}
...
function renderDetail(item){
  ...
  bindSeriesArchive(item);bindBrokenImages(els.detail);bindPersonLinks(els.detail);bindGuideImages(els.detail);bindLeopelGallery(els.detail,item);bindRelated();
}
