...
function renderParticipantGroups(groups=[]){
  ...
  return `<div class="archive-participant-groups">${[...byTitle.entries()].map(([title,rows])=>{
    ...
    return `<details class="archive-participant-wave"><summary><span><b>${escapeHtml(title)}</b><small>${count?count+'명':'명단 확인 중'}</small></span><i>명단 보기</i></summary><div class="archive-participant-wave-body">${rows.map(row=>`<section><header><strong>${escapeHtml(row.platform||'참가자')}</strong><span>${Number(row.count||row.participants?.length||0)}명</span></header>${row.note?`<p>${escapeHtml(row.note)}</p>`:''}<div class="archive-people-chips">${(row.participants||[]).map(name=>`<span>${escapeHtml(name)}</span>`).join('')}</div></section>`).join('')}</div></details>`;
  }).join('')}</div>`;
}
