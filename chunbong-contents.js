...
function isLeopel(item){return String(item?.id||'')==='leopel'||String(item?.series?.id||'')==='leopel'}
function leopelKnowledgeModel(item){
  if(!isLeopel(item))return null;
  return {
    overview:[
      ['기간','2025.06.05 ~ 07.06 · 32일'],
      ['셧다운','06:00 ~ 17:00 · 11시간'],
      ['플랫폼','SOOP · 치지직 · YouTube'],
      ['장르','RP · 어드벤처 · 서바이벌'],
      ['최종 참여자',String(item.participantCount||671)+'명'],
      ['스폰서','인텔 · 피씨디렉트 · 바삭팝콘']
    ],
    story:[
      ['기억을 잃은 모험가','플레이어는 지구와 다른 차원 레오펠에 소환된 모험가가 되어 잃어버린 기억과 자신을 찾는 여정을 시작합니다.'],
      ['무너진 신비의 동상','레오펠을 지키던 신비의 동상이 파괴되며 차원이 열렸고, 동상을 복원하는 것이 세계를 구하고 원래 세계로 돌아가기 위한 공동 목표가 됩니다.'],
    ...
    identity:[
      ['이름의 의미','Leo(사자)와 Fel에서 출발해, 이야기·말씀의 의미를 담은 Pel로 변형한 창작 고유명사 LEOPEL로 정리됩니다.'],
      ['제작 취지','춘봉이 다른 서버에서 새로운 인연과 방송의 전환점을 얻었던 경험을 바탕으로, 더 많은 방송인이 서로 만나 새로운 이야기를 만들 수 있는 3사 통합 서버를 목표로 기획했습니다.'],
      ['서버 방향','경쟁보다 협동과 힐링, RP와 모험을 중심에 두고 참가자의 플레이가 전체 이야기와 엔딩에 연결되는 구조를 지향했습니다.']
    ]
  };
}
function renderLeopelKnowledge(item,{compact=false}={}){
  const model=leopelKnowledgeModel(item);if(!model)return'';
  const gallery=(item.gallery||[]).filter(row=>row.src).slice(0,compact?2:4);
  return `<section class="archive-knowledge-system archive-leopel-knowledge ${compact?'is-compact':''}">
    <div class="archive-section-heading"><div><small>LEOPEL WORLD GUIDE</small><h3>레오펠 한눈에 보기</h3><p>나무위키의 레오펠 문서 구조를 바탕으로 SOOP 공식 기록과 교차해 세계관·시스템·진행 흐름을 읽기 쉽게 재구성했습니다.</p></div><span>사자의 노래</span></div>
    <div class="archive-fact-grid">${model.overview.map(([label,value])=>`<article><small>${escapeHtml(label)}</small><strong>${escapeHtml(value)}</strong></article>`).join('')}</div>
...
function knowledgeSections(item){return (item.knowledgeSections||[]).filter(row=>row&&(row.title||row.description||(row.cards||[]).length||(row.flow||[]).length||(row.chips||[]).length||(row.images||[]).length))}
function knowledgeImageMarkup(image={},item={}){
...
function renderKnowledgeSection(section,item,{compact=false}={}){
  ...
  return `<section class="archive-knowledge-section is-${escapeHtml(section.kind||'section')}"><div class="archive-section-heading"><div><small>${escapeHtml(section.eyebrow||'ARCHIVE GUIDE')}</small><h3>${escapeHtml(section.title||'가이드')}</h3>${section.description?`<p>${escapeHtml(section.description)}</p>`:''}</div></div>${cards.length?`<div class="archive-knowledge-grid">${cards.map(card=>`<article class="archive-knowledge-card"><small>${escapeHtml(card.eyebrow||'GUIDE')}</small><strong>${escapeHtml(card.title||'')}</strong>${card.value?`<b class="archive-knowledge-value">${escapeHtml(card.value)}</b>`:''}${card.text?`<p>${escapeHtml(card.text)}</p>`:''}${(card.items||[]).length?`<ul>${card.items.map(value=>`<li>${escapeHtml(value)}</li>`).join('')}</ul>`:''}</article>`).join('')}</div>`:''}${chips.length?`<div class="archive-system-chip-grid">${chips.map((chip,i)=>`<span><b>${String(i+1).padStart(2,'0')}</b>${escapeHtml(chip)}</span>`).join('')}</div>`:''}${flow.length?`<div class="archive-flow archive-knowledge-flow">${flow.map((step,i)=>`<span><b>${i+1}</b><em>${escapeHtml(step.title||'')}</em>${step.text?`<small>${escapeHtml(step.text)}</small>`:''}</span>`).join('')}</div>`:''}${images.length?`<div class="archive-knowledge-images ${images.length===1?'is-single':''}">${images.map(image=>knowledgeImageMarkup(image,item)).join('')}</div>`:''}</section>`;
}
function renderKnowledgeArchive(item,{compact=false}={}){
  const rows=knowledgeSections(item);if(!rows.length)return'';
  const shown=compact?rows.slice(0,3):rows;
  return `<div class="archive-knowledge-archive ${compact?'is-compact':''}">${shown.map(row=>renderKnowledgeSection(row,item,{compact})).join('')}${compact&&rows.length>shown.length?`<p class="archive-knowledge-more">가이드 탭에서 ${rows.length}개 섹션 전체를 확인할 수 있습니다.</p>`:''}</div>`;
}
...
function bindLeopelGallery(root,item){
  ...
  root.querySelectorAll('[data-leopel-gallery]').forEach(button=>button.addEventListener('click',()=>{
    const row=rows[Number(button.dataset.leopelGallery)];if(!row)return;
    const source=sourceMap(item).get(row.sourceId);openLightbox(row.src,row.alt||row.caption||'레오펠 자료 이미지',source?.url||'',row.caption||row.alt||'');
  }));
}
...
function activateTab(name,item){els.detail.querySelectorAll('[data-archive-tab]').forEach(b=>b.setAttribute('aria-selected',String(b.dataset.archiveTab===name)));const panel=els.detail.querySelector('[data-archive-panel]'),renderers={overview:renderOverview,guide:renderArchiveGuide,timeline:renderTimeline,media:renderMedia,posts:renderPosts,people:renderPeople,gallery:renderGallery,sources:renderSources};panel.innerHTML=(renderers[name]||renderOverview)(item);bindBrokenImages(panel);bindPersonLinks(panel);bindGuideImages(panel);bindKnowledgeImages(panel);bindLeopelGallery(panel,item);if(name==='media')bindMediaFilter(panel);if(name==='people')bindPeoplePanel(panel);if(name==='gallery')bindLightbox(item)}
