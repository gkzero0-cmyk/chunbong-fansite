...
function renderParticipantGroups(item){
  ...
  return `<div class="archive-participant-groups">${groups.map(group=>`<details class="archive-participant-group"><summary><span><b>${escapeHtml(group.title||group.platform||'참가자 그룹')}</b>${group.platform?`<small>${escapeHtml(group.platform)}</small>`:''}</span><strong>${Number(group.count||group.participants?.length||0)}명</strong></summary>${group.note?`<p>${escapeHtml(group.note)}</p>`:''}${(group.participants||[]).length?`<div class="archive-people-chips">${group.participants.map(name=>`<span>${escapeHtml(name)}</span>`).join('')}</div>`:'<p>확인된 명단을 정리하고 있습니다.</p>'}</details>`).join('')}</div>`;
}
function renderPeople(item){const p=item.participants||[],g=item.participantGroups||[],r=item.results||[],isClass=item.category==='class-event',groupCount=g.reduce((sum,row)=>sum+Number(row.count||row.participants?.length||0),0);return`<div class="archive-panel"><div class="archive-section-heading"><div><small>${isClass?'CLASS RECORD':'PEOPLE & RESULT'}</small><h2>${isClass?'수강생 · 회차 기록':'참가자 · 결과'}</h2></div><span>${p.length?p.length+'명':groupCount?groupCount+'명 확인':'확인 중'}</span></div>${p.length?`<div class="archive-people-chips">${p.map(name=>`<span>${escapeHtml(name)}</span>`).join('')}</div>`:g.length?'':`<p>${isClass?'확인된 수강생 명단을 정리하고 있습니다.':'공식 자료에서 확인된 참가자 명단을 정리하고 있습니다.'}</p>`}${renderParticipantGroups(item)}${r.length?`<div class="archive-results-grid">${r.map(x=>`<article><small>${isClass?'SESSION':'RESULT'}</small><strong>${escapeHtml(x.title||x.label||'결과')}</strong><p>${escapeHtml(x.value||x.name||'')}</p></article>`).join('')}</div>`:''}</div>`}
...
function renderSources(item){const rows=[...(item.sources||[])].sort((a,b)=>sourcePriority(a)-sourcePriority(b)||String(a.label||'').localeCompare(String(b.label||''),'ko'));return`<div class="archive-panel"><div class="archive-section-heading"><div><small>SOURCE ARCHIVE</small><h2>출처</h2></div><span>${rows.length}개</span></div><p class="archive-source-guide">춘봉 SOOP 방송국을 최우선으로, 춘봉TV YouTube와 공식 자료를 다음 순서로 표시합니다. 내부 검증용 자료는 이 화면에 노출하지 않습니다.</p><div class="archive-source-list">${rows.map(r=>`<a href="${escapeHtml(safeUrl(r.url))}" target="_blank" rel="noreferrer"><span><strong>${escapeHtml(r.label||'원문 자료')}</strong><small class="archive-source-kind is-${escapeHtml(r.kind||'reference')}">${escapeHtml(sourceKindLabel(r.kind))}</small></span><b>↗</b></a>`).join('')}</div></div>`}
