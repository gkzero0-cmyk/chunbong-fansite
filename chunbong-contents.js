...
function filterItems(items,{q='',category='all',year='all',series=''}={}){const n=normalize(q);return(Array.isArray(items)?items:[]).filter(item=>{if(series&&itemSeriesId(item)!==series)return false;if(category!=='all'&&item.category!==category)return false;if(year!=='all'&&!String(item.startDate||'').startsWith(year))return false;if(!n)return true;const grouped=(item.participantGroups||[]).flatMap(group=>group.participants||[]);return normalize([item.title,item.series?.title,item.series?.subtitle,...(item.aliases||[]),...(item.participants||[]),...grouped].join(' ')).includes(n)})}
...
function queryState(){const p=new URLSearchParams(global.location.search);return{id:p.get('id')||'',session:p.get('session')||'',series:p.get('series')||'',q:p.get('q')||'',category:p.get('category')||'all',year:p.get('year')||'all',sort:p.get('sort')==='oldest'?'oldest':'newest'}}
...
function seriesGroupVisual(group){
  ...
  return '<div class="archive-series-card-visual '+(rows.length>1?'is-multi':'')+'">'+rows.map((row,i)=>'<div class="archive-series-card-image">'+imageMarkup(row,row.alt,i===0)+'</div>').join('')+'</div>';
}
...
function renderList(){const s=queryState();syncControls(s);renderSeriesNavigation(s);const rows=sortItems(filterItems(allItems,s),s.sort);els.list.innerHTML=rows.map(cardMarkup).join('');bindBrokenImages(els.list);els.empty.hidden=rows.length>0;els.count.textContent=allItems.length?`전체 ${allItems.length}개 · 현재 ${rows.length}개`:'검증된 콘텐츠 기록을 준비하고 있습니다.';els.list.querySelectorAll('[data-archive-open]').forEach(a=>a.addEventListener('click',e=>{e.preventDefault();writeState({...s,id:a.dataset.archiveOpen},{push:true});void renderRoute()}))}
function resetFilters(){writeState({id:'',session:'',series:'',q:'',category:'all',year:'all',sort:'newest'});syncControls(queryState());renderList()}
...
function renderRecordStrip(item){
  ...
  const rows=[
    ...
    [peopleLabel,(item.participants||[]).length?item.participants.length+'명':'확인 중'],
...
function renderOverview(item){
  const people=(item.participants||[]).slice(0,16);
  const peopleTitle=item.category==='class-event'?'확인된 수강생':'확인된 참가자';
  return `<div class="archive-panel archive-overview-panel"><div class="archive-section-heading"><div><small>ABOUT THIS CONTENT</small><h2>콘텐츠 소개</h2></div></div><p class="archive-description">${escapeHtml(item.description||item.summary||'공식 자료를 기반으로 내용을 정리하고 있습니다.')}</p><div class="archive-subheading"><small>SUMMARY</small><h3>주요 기록</h3></div>${renderRecordStrip(item)}${people.length?`<section class="archive-overview-people"><div class="archive-subheading"><small>PEOPLE</small><h3>${escapeHtml(peopleTitle)}</h3></div><div class="archive-people-chips">${people.map(name=>`<span>${escapeHtml(name)}</span>`).join('')}</div></section>`:''}${renderOverviewHighlights(item)}</div>`;
}
...
function renderMedia(item){
  ...
  const groups=order.map(type=>[type,rows.filter(row=>row.type===type)]).filter(([,list])=>list.length);
  const known=new Set(order),other=rows.filter(row=>!known.has(row.type));if(other.length)groups.push(['other',other]);
  return `<div class="archive-panel"><div class="archive-section-heading"><div><small>MEDIA ARCHIVE</small><h2>영상 · 방송</h2><p>영상 종류별로 묶어 필요한 기록을 빠르게 찾을 수 있습니다.</p></div><span>${rows.length}개</span></div><div class="archive-media-groups">${groups.map(([type,list])=>`<details class="archive-media-group" ${list.length<=8?'open':''}><summary><span><b>${escapeHtml(type==='other'?'기타':materialTypeLabel(type))}</b><small>${list.length}개</small></span><i>펼쳐보기</i></summary><div class="archive-media-grid">${list.map(mediaCardMarkup).join('')}</div></details>`).join('')}</div></div>`;
}
...
function renderParticipantGroups(groups=[]){
  ...
  groups.forEach(group=>{const key=group.title||'참가자';if(!byTitle.has(key))byTitle.set(key,[]);byTitle.get(key).push(group)});
  return `<div class="archive-participant-groups">${[...byTitle.entries()].map(([title,rows])=>{
    const count=rows.reduce((sum,row)=>sum+Number(row.count||row.participants?.length||0),0);
    return `<details class="archive-participant-wave"><summary><span><b>${escapeHtml(title)}</b><small>${count?count+'명':'명단 확인 중'}</small></span><i>명단 보기</i></summary><div class="archive-participant-wave-body">${rows.map(row=>`<section><header><strong>${escapeHtml(row.label||row.platform||'참가자')}</strong><span>${Number(row.count||row.participants?.length||0)}명</span></header>${row.note?`<p>${escapeHtml(row.note)}</p>`:''}<div class="archive-people-chips">${(row.participants||[]).map(name=>`<span>${escapeHtml(name)}</span>`).join('')}</div></section>`).join('')}</div></details>`;
  }).join('')}</div>`;
}
function renderPeople(item){const p=item.participants||[],groups=item.participantGroups||[],r=item.results||[],isClass=item.category==='class-event';const groupedCount=groups.reduce((sum,row)=>sum+Number(row.count||row.participants?.length||0),0);const displayCount=groupedCount||p.length;return`<div class="archive-panel"><div class="archive-section-heading"><div><small>${isClass?'CLASS RECORD':'PEOPLE & RESULT'}</small><h2>${isClass?'수강생 · 회차 기록':'참가자 · 결과'}</h2></div><span>${displayCount?displayCount+'명 확인':'확인 중'}</span></div>${groups.length?renderParticipantGroups(groups):(p.length?`<div class="archive-people-chips">${p.map(name=>`<span>${escapeHtml(name)}</span>`).join('')}</div>`:`<p>${isClass?'확인된 수강생 명단을 정리하고 있습니다.':'확인된 자료에서 참가자 명단을 정리하고 있습니다.'}</p>`)}${r.length?`<div class="archive-results-grid">${r.map(x=>`<article><small>${isClass?'SESSION':'RESULT'}</small><strong>${escapeHtml(x.title||x.label||'결과')}</strong><p>${escapeHtml(x.value||x.name||'')}</p></article>`).join('')}</div>`:''}</div>`}
...
function renderSources(item){const rows=item.sources||[];return`<div class="archive-panel"><div class="archive-section-heading"><div><small>SOURCE ARCHIVE</small><h2>출처</h2></div><span>${rows.length}개</span></div><p class="archive-source-guide">직접 확인할 가치가 있는 공식 게시글·영상과 공개 참고자료만 표시합니다. 내부 검증용 자료는 이 화면에 노출하지 않습니다.</p><div class="archive-source-list">${rows.map(r=>`<a href="${escapeHtml(safeUrl(r.url))}" target="_blank" rel="noreferrer"><span><strong>${escapeHtml(r.label||'원문 자료')}</strong><small class="archive-source-kind is-${escapeHtml(r.kind||'reference')}">${escapeHtml(sourceKindLabel(r.kind))}</small></span><b>↗</b></a>`).join('')}</div></div>`}
...
function renderSeriesArchive(item){
  ...
  const poster=posterSrc
    ?`<button type="button" class="archive-series-poster is-ready" data-archive-series-poster aria-label="${escapeHtml(selected.title||'회차')} 포스터 크게 보기"><img src="${escapeHtml(posterSrc)}" alt="${escapeHtml(selected.poster?.alt||selected.title||'춘타클 회차 포스터')}" loading="eager" decoding="async" fetchpriority="high"><span>포스터 크게 보기</span></button>`
  ...
  return `<section class="archive-series" data-archive-series><div class="archive-series-heading"><div><small>SERIES ARCHIVE</small><h2>춘타클 회차 기록</h2><p>제1회부터 제5회까지 날짜·시간·수강생·포스터를 회차별로 분리해 확인할 수 있습니다.</p></div><span>${rows.length}회차</span></div><div class="archive-series-nav" role="tablist" aria-label="춘타클 회차 선택">${rows.map(row=>`<button type="button" role="tab" data-archive-session="${Number(row.number)}" aria-selected="${String(Number(row.number)===Number(selected.number))}"><b>제${Number(row.number)}회</b><small>${escapeHtml(formatDate(row.date,row.datePrecision||'day').replace(/^2026년 /,''))}</small></button>`).join('')}</div><div class="archive-series-focus">${poster}<div class="archive-series-copy"><p class="kicker">SESSION ${String(selected.number).padStart(2,'0')}</p><h3>${escapeHtml(selected.title||`춘타클 제${selected.number}회`)}</h3><div class="archive-series-meta"><span>${escapeHtml(dateLabel)}</span>${selected.time?`<span>${escapeHtml(selected.time)}</span>`:''}${selected.venue?`<span>${escapeHtml(selected.venue)}</span>`:''}<span>수강생 ${escapeHtml(attendance)}</span></div>${selected.note?`<p class="archive-series-note">${escapeHtml(selected.note)}</p>`:''}${people.length?`<div class="archive-series-people"><small>확인된 수강생</small><div class="archive-people-chips">${people.map(name=>`<span>${escapeHtml(name)}</span>`).join('')}</div></div>`:'<p class="archive-series-unconfirmed">수강생 명단은 확인되는 항목만 추가합니다.</p>'}</div></div></section>`;
}
...
function renderDetail(item){const hero=item.heroImage?.src?`<div class="archive-detail-media">${imageMarkup(item.heroImage,item.title,true)}</div>`:'<div class="archive-detail-media is-fallback"></div>';const isClass=item.category==='class-event',peopleLabel=isClass?'수강생':'참가자',peopleTab=isClass?'수강생 · 회차':'참가자 · 결과',seriesBack=queryState().series&&item.series?.title?item.series.title+' 시리즈':'콘텐츠 목록';els.detail.innerHTML=`<button type="button" class="archive-reset" data-archive-back>← ${escapeHtml(seriesBack)}</button>${renderSiblingSeriesNav(item)}<div class="archive-detail-hero">${hero}<div class="archive-detail-copy"><p class="kicker">${escapeHtml(categoryLabel(item.category))}</p><h1>${escapeHtml(item.title)}</h1><p>${escapeHtml(item.summary||'공식 자료를 기반으로 콘텐츠 기록을 정리했습니다.')}</p><div class="archive-detail-meta"><span class="archive-chip">${escapeHtml(statusLabel(item.status))}</span><span class="archive-chip">${escapeHtml(item.role||'주최')}</span><span class="archive-chip">${escapeHtml(formatRange(item))}</span></div></div></div>${renderSeriesArchive(item)}<div class="archive-summary-grid"><div><small>진행 기간</small><strong>${escapeHtml(formatRange(item))}</strong></div><div><small>${peopleLabel}</small><strong>${(item.participants||[]).length?item.participants.length+'명':'확인 중'}</strong></div><div><small>타임라인</small><strong>${(item.timeline||[]).length}건</strong></div><div><small>영상 · 방송</small><strong>${(item.media||[]).length}개</strong></div><div><small>자료 이미지</small><strong>${(item.gallery||[]).length}장</strong></div><div><small>출처</small><strong>${item.sourceCount||item.sources?.length||0}개</strong></div></div><div class="archive-tabs" role="tablist" aria-label="콘텐츠 기록 섹션"><button type="button" role="tab" data-archive-tab="overview" aria-selected="true">소개</button><button type="button" role="tab" data-archive-tab="timeline" aria-selected="false">타임라인</button><button type="button" role="tab" data-archive-tab="media" aria-selected="false">영상 · 방송</button><button type="button" role="tab" data-archive-tab="posts" aria-selected="false">게시글</button><button type="button" role="tab" data-archive-tab="people" aria-selected="false">${peopleTab}</button><button type="button" role="tab" data-archive-tab="gallery" aria-selected="false">자료 이미지</button><button type="button" role="tab" data-archive-tab="sources" aria-selected="false">출처</button></div><div data-archive-panel>${renderOverview(item)}</div>`;els.detail.querySelector('[data-archive-back]')?.addEventListener('click',()=>{writeState({...queryState(),id:'',session:''},{push:true});void renderRoute()});els.detail.querySelectorAll('[data-archive-tab]').forEach(b=>b.addEventListener('click',()=>activateTab(b.dataset.archiveTab,item)));els.detail.querySelectorAll('[data-archive-sibling]').forEach(b=>b.addEventListener('click',()=>{writeState({...queryState(),series:itemSeriesId(item),id:b.dataset.archiveSibling||'',session:''},{push:true});void renderRoute()}));bindSeriesArchive(item);bindBrokenImages(els.detail)}
