...
function renderPeople(item){const p=item.participants||[],r=item.results||[],isClass=item.category==='class-event';return`<div class="archive-panel"><div class="archive-section-heading"><div><small>${isClass?'CLASS RECORD':'PEOPLE & RESULT'}</small><h2>${isClass?'수강생 · 회차 기록':'참가자 · 결과'}</h2></div><span>${p.length?p.length+'명':'확인 중'}</span></div>${p.length?`<div class="archive-people-chips">${p.map(name=>`<span>${escapeHtml(name)}</span>`).join('')}</div>`:`<p>${isClass?'확인된 수강생 명단을 정리하고 있습니다.':'공식 자료에서 확인된 참가자 명단을 정리하고 있습니다.'}</p>`}${r.length?`<div class="archive-results-grid">${r.map(x=>`<article><small>${isClass?'SESSION':'RESULT'}</small><strong>${escapeHtml(x.title||x.label||'결과')}</strong><p>${escapeHtml(x.value||x.name||'')}</p></article>`).join('')}</div>`:''}</div>`}
