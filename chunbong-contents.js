...
function filterItems(items,{q='',category='all',status='all',year='all',series=''}={}){
  const n=normalize(q);
...
const d=global.document,$=s=>d.querySelector(s),els={browser:$('[data-archive-browser]'),list:$('[data-archive-list]'),detail:$('[data-archive-detail]'),search:$('[data-archive-search]'),category:$('[data-archive-category]'),status:$('[data-archive-status]'),year:$('[data-archive-year]'),sort:$('[data-archive-sort]'),count:$('[data-archive-count]'),empty:$('[data-archive-empty]'),lightbox:$('[data-archive-lightbox]'),seriesHome:$('[data-archive-series-home]'),seriesList:$('[data-archive-series-list]'),seriesLanding:$('[data-archive-series-landing]'),seriesCount:$('[data-archive-series-count]'),listHeading:$('[data-archive-list-heading]'),listCopy:$('[data-archive-list-copy]')};let allItems=[];
...
function queryState(){const p=new URLSearchParams(global.location.search);return{id:p.get('id')||'',session:p.get('session')||'',series:p.get('series')||'',q:p.get('q')||'',category:p.get('category')||'all',status:p.get('status')||'all',year:p.get('year')||'all',sort:p.get('sort')==='oldest'?'oldest':'newest'}}
function writeState(next,{push=false}={}){const p=new URLSearchParams();if(next.series)p.set('series',next.series);if(next.id)p.set('id',next.id);if(next.id&&next.session)p.set('session',next.session);if(next.q)p.set('q',next.q);if(next.category&&next.category!=='all')p.set('category',next.category);if(next.status&&next.status!=='all')p.set('status',next.status);if(next.year&&next.year!=='all')p.set('year',next.year);if(next.sort==='oldest')p.set('sort','oldest');const url=global.location.pathname+(p.size?'?'+p.toString():'');(push?history.pushState:history.replaceState).call(history,null,'',url)}
function syncControls(s){if(els.search)els.search.value=s.q;if(els.category)els.category.value=s.category;if(els.status)els.status.value=s.status;if(els.year&&[...els.year.options].some(o=>o.value===s.year))els.year.value=s.year;if(els.sort)els.sort.value=s.sort}
...
function resetFilters(){writeState({id:'',session:'',series:'',q:'',category:'all',status:'all',year:'all',sort:'newest'});syncControls(queryState());renderList()}
function bindToolbar(){const update=()=>{const s=queryState();writeState({...s,id:'',q:els.search?.value||'',category:els.category?.value||'all',status:els.status?.value||'all',year:els.year?.value||'all',sort:els.sort?.value||'newest'});renderList()};els.search?.addEventListener('input',update);els.category?.addEventListener('change',update);els.status?.addEventListener('change',update);els.year?.addEventListener('change',update);els.sort?.addEventListener('change',update);d.querySelectorAll('[data-archive-reset]').forEach(b=>b.addEventListener('click',resetFilters))}
...
function bindPersonLinks(root=els.detail){
  root.querySelectorAll('[data-archive-person]').forEach(button=>button.addEventListener('click',()=>{
    const person=button.dataset.archivePerson||'';writeState({id:'',session:'',series:'',q:person,category:'all',status:'all',year:'all',sort:'newest'},{push:true});void renderRoute();global.scrollTo?.({top:0,behavior:'smooth'});
  }));
}
...
function renderDetail(item){
  const hero=item.heroImage?.src?`<div class="archive-detail-media">${imageMarkup(item.heroImage,item.title,true)}</div>`:'<div class="archive-detail-media is-fallback"></div>';
