(() => {
  ...
  function openTerminalRankingRegistration(){const state=currentState();if(!isTerminal(state))return false;resetTerminalControls(state);panel.hidden=false;if(!nicknameInput.value)nicknameInput.value=storageGet(nicknameKey,'');setStatus(`${difficulty()==='extreme'?'익스트림':difficulty()==='hard'?'하드':'노말'} 랭킹에 등록합니다.`);nicknameInput.focus();nicknameInput.select?.();return true;}
  ...
  async function submitTerminalRanking(){
    ...
    try{
      ...
      submittedKey=key;storageSet(nicknameKey,nickname.displayName);registerButton.disabled=true;registerButton.textContent='등록 완료 ✓';setStatus(`${nickname.displayName} 이름으로 랭킹 등록이 완료됐어요.`,'success');await App.loadRanking?.(mode(),difficulty());return true;
