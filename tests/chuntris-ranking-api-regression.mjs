...
const firstHard=await invoke({method:'POST',body:{mode:'hard',nickname:'하드춘봉',score:2200,lines:18,level:4,timeMs:45000}}); assert.equal(firstHard.statusCode,200); assert.equal(firstHard.body.updated,true); assert.equal(firstHard.body.mode,'hard');
const getHard=await invoke({method:'GET',query:{mode:'hard'}}); assert.equal(getHard.statusCode,200); assert.equal(getHard.body.entries[0].nickname,'하드춘봉');
