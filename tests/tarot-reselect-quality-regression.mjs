...
assert.deepEqual(descriptor, {
  ...
  pairSlot: 1,
  cardIndex: 1,
  url: 'https://res.cloudinary.com/lyppgyei/image/upload/c_crop,g_north_west,h_1488,w_898,x_898,y_0/f_auto/q_auto/chunbong-fansite/tarot-original/sheet-0.avif',
  sourceX: 0,
  sheetWidth: 898,
  sheetHeight: 1488,
  cellWidth: 898,
...
assert.deepEqual(tarot.cardArtworkDescriptor({ imageSheet: 5, imageSlot: 12 }), {
  ...
  pairSlot: 1,
  cardIndex: 77,
  url: 'https://res.cloudinary.com/lyppgyei/image/upload/c_crop,g_north_west,h_1488,w_898,x_10776,y_0/f_auto/q_auto/chunbong-fansite/tarot-original/sheet-5.avif',
  sourceX: 0,
  sheetWidth: 898,
  sheetHeight: 1488,
  cellWidth: 898,
...
assert.ok(script.includes('viewBox="0 0 898 1488"'), 'fallback SVG should preserve one uploaded original card region');
assert.ok(script.includes('f_auto/q_auto'), 'fallback artwork should use optimized Cloudinary delivery');
...
assert.ok(css.includes('.tarot-card-back.selected'), 'selected card backs must remain visibly selected and clickable');
