...
const noticePage = read('page-notice.js');
...
assert.ok(noticePage.includes('NOTICE_REFRESH_MS'), 'notice page should define an automatic refresh interval');
assert.match(noticePage, /setInterval\([\s\S]*renderNoticePage/, 'notice page should periodically refresh the latest notices');
assert.ok(noticePage.includes('notice-state-label'), 'notice toggles should expose an explicit expand/collapse state label');
assert.ok(noticePage.includes('setupNoticeImageZoom'), 'notice detail images should open in a lightbox');
