...
assert.match(sw, /chunbong-pwa-20260923-v32/,'performance cache strategy must advance the PWA cache');
...
assert.match(sw, /\['script','style'\][\s\S]*staleWhileRevalidate\(request, event\)/, 'scripts and styles should render from cache immediately and revalidate in the background');
...
assert.doesNotMatch(sw, /\/site-meta\.js/, 'non-critical metadata runtime should not block PWA installation');
assert.doesNotMatch(sw, /\/site-health\.js/, 'runtime health checks should not block PWA installation');
assert.match(sw, /\/mobile-site\.js/, 'PWA app shell must cache mobile-site.js');
assert.doesNotMatch(sw, /\/personal-hub\.js/, 'personal hub should load on demand instead of blocking PWA installation');
