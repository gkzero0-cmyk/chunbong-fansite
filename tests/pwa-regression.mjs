...
assert.match(sw, /chunbong-pwa-20260921-v21/,'timeline retirement must advance the PWA cache');
assert.match(sw, /\/offline\.html/);
assert.match(sw, /url\.pathname === '\/api\/content'/, 'selected public content endpoints should be available to offline cache');\nassert.match(sw, /url\.pathname\.startsWith\('\/api\/'\)/);
...
assert.match(sw, /\/myhub\.html/, 'PWA app shell must cache My Fan Hub');\nassert.match(sw, /mobile-app-enhancements\.js/, 'PWA app shell must cache mobile app enhancements');\nassert.match(sw, /periodicsync/, 'supported installed apps should expose background LIVE checks');
