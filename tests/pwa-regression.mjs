...
assert.match(sw, /\['script','style'\][\s\S]*staleWhileRevalidate\(request, event\)/, 'scripts and styles should use stale-while-revalidate for faster repeat navigation');
...
assert.doesNotMatch(sw, /\/personal-hub\.js/, 'personal hub runtime should load on demand instead of bloating the initial PWA install');
