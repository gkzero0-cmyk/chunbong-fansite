...
assert.doesNotMatch(sw, /\/personal-hub\.js/, 'personal hub runtime should be runtime-cached after use, not forced into first-install precache');
assert.doesNotMatch(sw, /\/myhub\.html/, 'My Fan Hub should be runtime-cached after navigation, not forced into first-install precache');
assert.doesNotMatch(sw, /\/tarot\.js/, 'heavy tarot runtime must stay out of first-install precache');
assert.doesNotMatch(sw, /\/data\.js/, 'data dashboard runtime must stay out of first-install precache');
assert.match(shell, /interactivePersonalPages/, 'shared shell must keep personal features immediate only on interactive pages');
assert.match(shell, /requestIdleCallback/, 'passive pages must defer personal hub work');
