...
assert.match(api,/Max-Age=31536000/,'1-year operator session missing');
...
assert.match(operatorJs,/\$\$\('\[data-days\]'\)\.forEach/,'all analytics period controls must be bound');
assert.doesNotMatch(operatorJs,/[^$]\$\('\[data-days\]'\)\.forEach/,'single-element selector must not bind period controls');
