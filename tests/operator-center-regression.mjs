...
assert.match(auth,/code_challenge/,'GitHub OAuth PKCE challenge missing');
assert.match(auth,/code_verifier/,'GitHub OAuth PKCE verifier missing');
assert.match(auth,/sameOrigin/,'operator POST auth actions must enforce same-origin requests');
assert.match(auth,/operator:email-auth-rate/,'operator email-link request rate limit missing');
