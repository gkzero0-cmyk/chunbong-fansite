...
const enhancedJs = fs.readFileSync(new URL('../tarot-sfx-v2.js', import.meta.url), 'utf8');
...
const audioSources = `${js}\n${enhancedJs}`;
for (const cue of ['shuffle','select','reveal','complete']) assert.ok(audioSources.includes(`play('${cue}')`));
assert.ok(enhancedJs.includes('createBuffer'), 'enhanced Tarot SFX must synthesize card noise with Web Audio');
assert.ok(!audioSources.match(/\.(mp3|wav|ogg)/i));
console.log('tarot effects and enhanced Web Audio regression test passed');
