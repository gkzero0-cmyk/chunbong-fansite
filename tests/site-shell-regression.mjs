...
for(const token of ['ChunbongCache','nav-group','theme-toggle','changelog-summary','activity-center.js']){
  assert.ok(shell.includes(token), 'shared shell missing '+token);
}
assert.ok(!shell.includes('window.CHUNBONG_CONTENT ='), 'shared shell must not carry page content data');
assert.ok(content.includes('window.CHUNBONG_CONTENT ='), 'content bundle must keep page content data');
...
for(const file of gamePages){
  const html=read(file);
  assert.match(html,/src="site-shell\.js"/,file+' missing shared shell');
  assert.doesNotMatch(html,/src="content\.js"/,file+' must not load general content data');
  assert.match(html,/src="page\.js"/,file+' must keep common page/PWA runtime');
}
