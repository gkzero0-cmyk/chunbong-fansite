...
for(const asset of ['/chunbong-contents.html','/chunbong-contents.css','/chunbong-contents.js']) assert.ok(!sw.includes("'"+asset+"'"),asset+' should runtime-cache after first visit instead of blocking PWA install');
...
for(const token of ['renderNotionGuide','renderNotionPreview','data-archive-tab="guide"','지식 가이드']) assert.ok(js.includes(token),token);
...
assert.ok(serviceWorker.includes("chunbong-pwa-"),'service worker cache namespace should remain compatible');
