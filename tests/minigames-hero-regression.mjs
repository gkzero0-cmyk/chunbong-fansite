...
const assetPath=new URL('../assets/minigames-hero.webp',import.meta.url);
...
assert.match(html,/src="assets\/minigames-hero\.webp\?v=20260919"/,'hero must use the stable root-level higher-resolution artwork');
assert.match(html,/data-fallback-src="assets\/minigames-hero\.svg"/,'hero must retain a local SVG fallback');
assert.match(html,/onerror="[^"]*dataset\.fallbackSrc/,'hero must switch to its fallback if WebP loading fails');
