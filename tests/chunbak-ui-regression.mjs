...
for(const id of ['chunbak-game','chunbak-start','chunbak-restart','chunbak-score','chunbak-best','chunbak-max-level','chunbak-ranking-status','chunbak-ranking-list','chunbak-stage','chunbak-canvas','chunbak-danger-line','chunbak-next','chunbak-stage-legend','chunbak-overlay','chunbak-ranking-register','chunbak-ranking-nickname']) assert.ok(html.includes(`id="${id}"`), `missing ${id}`);
assert.equal(html.includes('id="chunbak-nickname"'),false,'pre-game nickname input must be removed');
