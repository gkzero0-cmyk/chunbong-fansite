...
assert.match(apiSource,/guide-media-migration:v6-permanent-and-reference-proxy/,'guide-media migration marker must be versioned for the full archive refresh');
...
assert.match(apiSource,/assetState:'proxy'/,'Guide images must fall back to stable proxy state when permanent storage is unavailable');
assert.match(apiSource,/function referenceGuideImageProxyUrl/,'stable NamuWiki reference image proxy helper missing');
assert.match(apiSource,/async function handleReferenceGuideImage/,'NamuWiki reference image proxy handler missing');
