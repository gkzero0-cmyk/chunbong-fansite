...
assert.match(apiSource,/guide-media-migration:v3-media-diagnostics/,'guide-media migration marker must be versioned for the full archive refresh');
assert.match(apiSource,/method==='GET'&&requestUrl\.searchParams\.get\('migration'\)==='guide-media-v3'/,'one-time guide-media migration trigger missing');
...
assert.match(apiSource,/guide_media_migration_v3_complete/,'completed guide migration must be idempotent');
assert.match(apiSource,/AUTO_LOCK_KEY\+':guide-media-v3'/,'guide migration must use an execution lock');
