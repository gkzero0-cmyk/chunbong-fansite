...
const first=await invoke({method:'POST',body:{mode:'classic',nickname:'춘봉',score:1000,maxLevel:6}}); assert.equal(first.statusCode,200); assert.equal(first.body.updated,true); assert.equal(first.body.entries[0].nickname,'춘봉'); assert.equal(first.body.entries[0].score,1000);
const lower=await invoke({method:'POST',body:{mode:'classic',nickname:'춘봉',score:900,maxLevel:8}}); assert.equal(lower.body.updated,false); assert.equal(lower.body.entries[0].score,1000);
const sameHigherLevel=await invoke({method:'POST',body:{mode:'classic',nickname:'춘봉',score:1000,maxLevel:8}}); assert.equal(sameHigherLevel.body.updated,true); assert.equal(sameHigherLevel.body.entries[0].maxLevel,8);
...
const unsupported=await invoke({method:'DELETE',query:{mode:'classic'}}); assert.equal(unsupported.statusCode,405); assert.equal(unsupported.headers.allow,'GET, POST');
