...
assert.ok(html.includes('<link rel="stylesheet" href="tarot-bundle.css?v=1">'), 'tarot.html must ship vector-composite styles through the generated tarot bundle');
assert.ok(html.includes('<script src="tarot-bundle.js?v=1"></script>'), 'tarot.html must ship the vector-composite browser upgrader through the generated JS bundle');
...
assert.match(upright, /class="tarot-composite-art-layer" clip-path="url\(#test-upright-art-clip\)"/, 'the illustration clipping viewport must remain fixed');
assert.match(upright, /class="tarot-composite-art-viewport"/, 'one uploaded card slot must be isolated before compositing');
...
assert.match(reversed, /class="tarot-composite-art-rotation" transform="rotate\(180 480 656\)"/, 'reversed readings must rotate only the isolated raster illustration');
