...
assert.match(index,/href="changelog\.html"><strong>업데이트 일지/,'home should replace timeline with changelog shortcut');
assert.doesNotMatch(index,/timeline\.html|춘봉 타임라인/,'retired timeline must not remain on home');
assert.doesNotMatch(mobile,/timeline\.html|data-more-page="timeline"/,'retired timeline must not remain in mobile More menu');
...
assert.match(sw,/chunbong-pwa-20260920-v19/,'personal hub release must advance PWA cache');
