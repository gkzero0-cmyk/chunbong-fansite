...
assert.match(js, /const SPIN_MS = 3250/, 'progressive fortune spin must run for 3.25 seconds');
...
assert.match(js,/const DECEL_START_MS = 2450/,'progressive spin must enter its deceleration phase at the current timing');
...
assert.match(js,/stage\.addEventListener\('pointerup',[\s\S]*startDraw\(point\)/,'stage pointerup must start the draw with the pointer origin without depending on button click delivery');
assert.match(js,/cardButton\.addEventListener\('click',[\s\S]*event\.detail !== 0/,'button click must ignore pointer-generated clicks and remain as keyboard and assistive-tech fallback');
