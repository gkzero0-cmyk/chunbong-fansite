...
assert.match(operatorContents,/bindSourceMeta\(\);bindMaterialMeta\(\);\s*\$\$\('\[data-add-row\]'/,'archive add-row controls must remain a node list');
...
assert.equal(browserApplied.timeline[0]?.visibility,'internal','protected SOOP browser material must remain internal until explicitly reviewed');
assert.equal(browserApplied.timeline[0]?.url,'','protected post URL must not leak from the internal timeline material');
