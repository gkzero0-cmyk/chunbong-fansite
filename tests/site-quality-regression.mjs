...
const htmlPages = [
  'index.html','schedule.html','notice.html','vod.html','clips.html','fanart.html','youtube.html','tarot.html',
...
for (const {file,html} of htmlPages) {
  assert.match(html,/<!-- social meta -->/,file+' static social metadata missing');
  assert.match(html,/rel="canonical"/,file+' canonical link missing');
  assert.match(html,/property="og:title"/,file+' Open Graph title missing');
  assert.match(html,/name="twitter:card"/,file+' Twitter card metadata missing');
  assert.match(html,/site-quality\.css" data-site-quality/,file+' shared accessibility stylesheet missing');
}
