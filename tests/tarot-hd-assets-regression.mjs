...
for (const [index, asset] of assets.entries()) {
  ...
  assert.ok(bytes.length > 150 * 1024, `sheet ${index} should contain high-detail artwork`);
...
assert.ok(totalBytes > 1000 * 1024, 'HD sheets should contain substantially more detail than the old sprites');
assert.ok(totalBytes < 1400 * 1024, 'HD sheets should remain practical for result-time loading');
