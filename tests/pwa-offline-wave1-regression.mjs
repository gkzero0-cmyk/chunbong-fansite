...
for (const asset of [
  ...
  assert.ok(appShell.includes("'"+asset+"'"), 'core offline shell missing '+asset);
}
for (const featureAsset of [
  '/schedule.html','/tarot.html','/tarot.js','/data.html','/data.js',
  '/vod.html','/clips.html','/youtube.html','/fanart.html'
]) {
  assert.ok(!appShell.includes("'"+featureAsset+"'"), 'heavy feature asset should be runtime-cached, not install-precached: '+featureAsset);
}
