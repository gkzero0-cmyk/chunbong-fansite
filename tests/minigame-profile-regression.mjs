...
assert.match(sw,/chunbong-pwa-20260921-v21/,'PWA cache must advance for minigame profile');
...
assert.match(js,/ChunbongPersonal\?\.dailyChallenge/,'profile must render the daily challenge');\nassert.match(js,/chunbong\.minigame\.nickname\.v1/,'unified minigame nickname key missing');\nassert.match(js,/function setNickname/,'four-game nickname synchronizer missing');\nassert.match(js,/KEYS\.nickname\.forEach/,'nickname changes must fan out to all four games');
