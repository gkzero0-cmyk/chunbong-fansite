...
assert.ok(tarot.includes('c_crop,g_north_west'), 'runtime must use individual Cloudinary card crops');
assert.ok(tarot.includes('q_100/f_avif'), 'runtime must preserve maximum-quality AVIF delivery');
assert.ok(composite.includes('originalCardCropUrl'), 'composite renderer must use original card crops');
