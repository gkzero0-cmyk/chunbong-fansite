(() => {
  ...
  function buildRollingWeekOptions(rows = [], todayKey = '') {
    ...
    return Array.from({length:Math.max(1,Math.floor(diff/7)+1)},(_,offset)=>{const end=shift(today,-offset*7),start=shift(end,-6);return {offset,start,end,label:offset===0?'최근 7일':`${start.slice(5).replace('-','.')} ~ ${end.slice(5).replace('-','.')}`};});
  }
  ...
  window.__CHUNBONG_SOOP_PERIOD_HELPERS__={buildRollingWeekOptions,filterDailyByWeek,availableMonthKeys};
