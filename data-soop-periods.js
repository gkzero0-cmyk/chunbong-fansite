(() => {
  ...
  function renderDaily(payload) {
    ...
    let cumulative=0;
    const rows=weekRows(all,selected).map(row=>({...row,cumulativeMinutes:(cumulative+=Number.isFinite(row.durationMinutes)?row.durationMinutes:0)}));
    const favorite=rows.map(row=>({date:row.date,value:Number.isFinite(row.followerDelta)?row.followerDelta:null}));
    const fanclub=rows.map(row=>({date:row.date,value:Number.isFinite(row.fanclubDelta)?row.fanclubDelta:null}));
    ...
    if(chartRoot) chartRoot.innerHTML=[
      ...
      chart({title:'애청자 · 즐겨찾기 증감',rows:favorite,key:'value',formatter:signed,kind:'signed'}),
      chart({title:'팬클럽 증감',rows:fanclub,key:'value',formatter:signed,kind:'signed'})
    ].join('');
  ...
  function renderMonthly(payload) {
    ...
    if(root){
      root.innerHTML=months.map(month=>`<button type="button" class="data-period-control ${month===state.selectedMonth?'is-active':''}" data-soop-month-value="${month}">${Number(month.slice(5))}월</button>`).join('');
  ...
  style.textContent='.data-period-controls{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 18px}.data-period-control{appearance:none;border:1px solid rgba(255,255,255,.12);border-radius:999px;background:rgba(255,255,255,.04);color:#a6a6a6;font:900 13px/1 inherit;padding:10px 15px;cursor:pointer;transition:.18s ease}.data-period-control:hover{color:#fff;border-color:rgba(255,107,24,.5)}.data-period-control.is-active{color:#fff;background:#ff6417;border-color:#ff6417;box-shadow:0 5px 16px rgba(255,100,23,.2)}.data-category-period{display:grid;gap:10px;margin-bottom:24px}.data-category-period-head{display:flex;align-items:flex-end;justify-content:space-between;gap:12px;padding:0 2px 4px}.data-category-period-head strong{display:block;color:#fff;font-size:18px}.data-category-period-head small{display:block;margin-top:5px;color:#888;font-size:12px}@media(max-width:760px){.data-period-control{padding:9px 12px;font-size:12px}.data-category-period-head strong{font-size:16px}}';
