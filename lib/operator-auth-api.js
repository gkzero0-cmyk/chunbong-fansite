...
async function githubStart(req,res){
  ...
  setHeader(res,'Set-Cookie',cookie(STATE_COOKIE,state,{maxAge:600}));
...
async function githubCallback(req,res){
  ...
  if(!state||!code||!expected||!safeEqual(state,expected))return redirect(res,baseUrl()+'/operator.html?auth=state-error');
  try{
    const exchange=await fetch('https://github.com/login/oauth/access_token',{
      method:'POST',headers:{accept:'application/json','content-type':'application/json'},
      body:JSON.stringify({client_id:process.env.GITHUB_OAUTH_CLIENT_ID,client_secret:process.env.GITHUB_OAUTH_CLIENT_SECRET,code,redirect_uri:githubCallbackUrl()})
    });
    ...
    setHeader(res,'Set-Cookie',[cookie(SESSION_COOKIE,token),clearCookie(STATE_COOKIE)]);
...
async function emailComplete(req,res){
  ...
  try{
    ...
    if(!response.ok||!ownerEmailMatches(payload?.email||email)||!payload?.idToken)throw new Error('invalid_email_link');
...
async function handleAuth(req,res){
  ...
  if(action==='logout'&&method==='POST'){setHeader(res,'Set-Cookie',clearCookie(SESSION_COOKIE));return sendJson(res,200,{ok:true});}
