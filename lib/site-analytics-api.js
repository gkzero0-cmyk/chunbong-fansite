...
async function expire(...keys){await Promise.all(keys.map(key=>redisCommand('EXPIRE',key,RETENTION_SECONDS).catch(()=>null)));}
async function touchSession(payload,{visitorHash,sessionHash,now,path,date}){
  ...
  await redisCommand('EXPIRE',SESSION_INDEX,RETENTION_SECONDS);
...
async function recordEvent(payload,now=new Date()){
  ...
  await expire(summary,visitors,sessions,pages,pageMs,menu,features,devices,pwa,themes,hours,transitions,DAYS_KEY,ACTIVE_KEY);
  return {ok:true};
}
