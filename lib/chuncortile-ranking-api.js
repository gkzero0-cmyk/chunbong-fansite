...
const KEY_BY_MODE=Object.freeze({classic:'chuncortile:classic:players'});
...
function parseMode(value){return Object.prototype.hasOwnProperty.call(KEY_BY_MODE,value)?value:null;}
...
function publicEntry(record,rank){return{rank,nickname:record.displayName,score:record.score,maxCombo:record.maxCombo,misses:record.misses,achievedAt:record.achievedAt};}
...
module.exports=async function handler(req,res){
  ...
  try{
    if(method==='GET'){
      const mode=parseMode(req?.query?.mode);if(!mode)return sendJson(res,400,{error:'invalid_mode'});
    ...
    const record=validation.record,key=KEY_BY_MODE[record.mode],map=parseMap(await redisCommand('GET',key)),current=map[record.key]||null;
    const candidate={...record,achievedAt:new Date().toISOString()},updated=Core.isBetterRecord(record.mode,candidate,current);
    if(updated){map[record.key]=candidate;await redisCommand('SET',key,JSON.stringify(map));}
    const personalBest=updated?candidate:current;
    return sendJson(res,200,{mode:record.mode,updated,personalBest:personalBest?publicEntry(personalBest,null):null,entries:topEntries(record.mode,map)});
  }catch(error){
    console.error('[chuncortile-ranking] unavailable:',error?.message||'unknown');
