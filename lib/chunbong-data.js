...
const fetchRecentVod = require('./content-api/vod');
const fetchClipsDefault = require('./content-api/clips');
const fetchYoutubeDefault = require('./content-api/youtube');
const { SOOP_ID, getJson, listFrom, deepFirst, normalizeVideo } = require('./content-api/_shared');
...
const { fetchSoopStructuredLive, resolveLiveState, normalizeSoopPlayHtml } = require('./soop-live-state');
...
async function fetchSoopLive() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const parsedPlay = normalizeSoopPlayHtml(html, `https://play.sooplive.com/${SOOP_ID}`);
      htmlSignal = {
        ...parsedPlay,
        categoryId: parsedPlay.categoryId || metrics.categoryId,
        categoryName: parsedPlay.categoryName || metrics.categoryName
      };
    }
  } catch (_) {}
