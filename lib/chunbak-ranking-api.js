...
const KEY_BY_MODE = Object.freeze({ classic:'chunbak:classic:players' });
...
function parseBody(body) {
  ...
  if (typeof body === 'string' && body.length <= 4096) { try { return JSON.parse(body); } catch { return null; } }
  return null;
}
...
function publicEntry(record, rank) {
  return { rank, nickname:record.displayName, score:record.score, maxLevel:record.maxLevel, achievedAt:record.achievedAt };
}
...
module.exports = async function handler(req,res) {
  ...
  try {
    ...
    const key = KEY_BY_MODE[record.mode];
    ...
    console.error('[chunbak-ranking] unavailable:', error?.message || 'unknown');
