...
const MAX_PAGES=12;
const TECHNICAL_PREFIXES=new Set(['ci','test','tests','data','chore','build','docs','deps','dependabot']);
...
function compactCommit(commit={}){
  ...
  const date=/^20\d{2}-\d{2}-\d{2}/.test(rawDate)?rawDate.slice(0,10):'';
  return {
...
async function handler(req,res){
  ...
  try{
    ...
    const payload=summary?await loadSummary():await loadArchive();
...
module.exports._internals={firstLine,prefixOf,normalizeTitle,commitType,isBotCommit,isMeaningfulCommit,compactCommit,groupCommits,SITE_STARTED_AT};
