...
const INTERNAL_COMMIT_PATTERNS=[
  ...
  /(?:vercel|serverless|oidc|whatwg|urlsearchparams|deprecation|node\s*24|api\s*쿼리|쿼리\s*파싱|런타임\s*경고).*(?:전환|정리|제거|수정|보강|호환|파싱)/i
];
...
function isInternalCommit(message=''){
  const line=firstLine(message);
  return INTERNAL_COMMIT_PATTERNS.some(pattern=>pattern.test(line));
}
...
function isMeaningfulCommit(commit={}){
  ...
  if(isInternalCommit(message))return false;
...
module.exports._internals={firstLine,prefixOf,normalizeTitle,summaryOf,commitType,isInternalCommit,isBotCommit,isMeaningfulCommit,kstDate,compactCommit,groupCommits,SITE_STARTED_AT,TECHNICAL_PREFIXES,INTERNAL_COMMIT_PATTERNS};
