...
const AUTO_FULL_MIGRATION_KEY='content-archive:auto-sync:full-migration:v7-moneygame-participant-count-namuwiki-fallback-transport';
...
async function persistNotionImage(media={},itemId='content'){
  ...
  try{
    ...
    return permanent?{...media,src:permanent,originUrl:media.originUrl||src,sourceUrl:media.sourceUrl||'',provider:media.provider||'notion',permanent:true,assetState:'permanent',assetId:String(payload?.asset_id||''),publicId:String(payload?.public_id||'')} : media;
  }catch{return media}
}
...
async function refreshReferenceGuides(rows=[]){
  ...
  for(const item of rows){
    ...
    for(const source of sources){
      try{
        const meta=await fetchSourceMeta(source.url);
        const guideRows=referenceGuideRows(meta,source,'namuwiki');
        next.push(...guideRows);successCount++;
        details.push({
