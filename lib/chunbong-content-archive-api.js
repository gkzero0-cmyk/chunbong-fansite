...
function notionBlockMedia(block={}){
  ...
  return{src,sourceUrl:src,caption,filename,alt:caption||filename||'Notion 이미지',originalId:String(block.id||'')};
}
...
function notionStructuredSections(pages=[]){
  ...
  for(const page of pages){
    ...
    const root=rows[0]||{},blocks=[];let current={title:String(root.text||'').trim()||'본문',lines:[],images:[],content:[]};
    const push=()=>{
      if(current.title||current.lines.length||current.images.length)blocks.push({
        title:current.title||'본문',
        lines:current.lines.slice(0,200),
        text:current.lines.join('\n').slice(0,12000),
        images:current.images.slice(0,12),
    ...
    for(const row of rows.slice(1)){
      ...
      if(row?.media){
        current.images.push(row.media);
        current.content.push({type:'image',image:row.media});
        // Image/file block titles are usually filenames; do not repeat them as body text.
        continue;
      }
      if(row?.text){const value=String(row.text).trim();current.lines.push(value);current.content.push({type:'text',text:value})}
    }
    push();
...
function extractNamuStructured(html='',sourceUrl=''){
  ...
  const matches=[...source.matchAll(headingRe)],sections=[];
  const pushSection=(title,body,index)=>{
    const text=htmlPlainText(body).slice(0,5000);
    const images=[];const seen=new Set();
    for(const match of body.matchAll(/<img\b([^>]+)>/gi)){
      const attrs=match[1]||'';
      const raw=(attrs.match(/(?:data-src|src)=["']([^"']+)["']/i)||[])[1]||'';
      const src=htmlAbsoluteUrl(raw,sourceUrl);if(!src||seen.has(src)||/^data:/i.test(src))continue;seen.add(src);
      const alt=decodeHtml((attrs.match(/alt=["']([^"']*)["']/i)||[])[1]||'').replace(/\s+/g,' ').trim();
      images.push({id:'namu-image-'+index+'-'+images.length,src,alt:alt||title||'나무위키 자료 이미지',caption:alt||'',sourceUrl,sourceId:'source-reference',layout:'auto'});
      if(images.length>=12)break;
    }
    if(title||text||images.length)sections.push({id:'auto-namu-'+index,kind:'reference',eyebrow:'NAMU.WIKI',title:title||'레오펠',description:text,sourceId:'source-reference',cards:[],flow:[],chips:[],images});
  };
  if(matches.length){
    for(let i=0;i<matches.length;i++){
      const title=htmlPlainText(matches[i][2]);
      const start=(matches[i].index||0)+matches[i][0].length;
      const end=i+1<matches.length?(matches[i+1].index||source.length):source.length;
      pushSection(title,source.slice(start,end),i+1);
      if(sections.length>=48)break;
    }
  }else pushSection(readTitle(source),source,1);
  const allImages=sections.flatMap(row=>row.images||[]);
  return{sections,images:allImages.slice(0,80),contentText:sections.map(row=>[row.title,row.description].filter(Boolean).join('\n')).join('\n').slice(0,60000)};
}
function fetchNamuMeta(url,html){
  ...
  return{url:url.toString(),title:readTitle(html).slice(0,200),description:(readMeta(html,'og:description')||structured.contentText).slice(0,500),image:imageRaw,sections:structured.sections,images:structured.images,contentText:structured.contentText,strategy:'namuwiki-structured-html'};
}
...
function cloudinaryArchiveKey(media={}){
  ...
  const raw=String(media.sourceUrl||media.src||'');
...
async function persistNotionImage(media={},itemId='content'){
  ...
  const folder='chunbong-fansite/notion/'+String(itemId||'content').toLowerCase().replace(/[^a-z0-9가-힣-]+/g,'-').replace(/^-|-$/g,'').slice(0,80);
  const timestamp=Math.floor(Date.now()/1000);
  const signed={folder,overwrite:'true',public_id:publicId,timestamp:String(timestamp),unique_filename:'false'};
  const form=new FormData();
  form.set('file',src);form.set('api_key',config.apiKey);
  ...
  try{
    ...
    return permanent?{...media,src:permanent,sourceUrl:media.sourceUrl||src,permanent:true,assetId:String(payload?.asset_id||''),publicId:String(payload?.public_id||'')} : media;
  }catch{return media}
}
async function persistNotionGuideMedia(rows=[],itemId='content'){
  ...
  const persist=async media=>{
    ...
    const key=String(media.originalId||media.sourceUrl||media.src);
    if(!cache.has(key))cache.set(key,persistNotionImage(media,itemId));
    return cache.get(key);
  };
  const out=[];
  for(const row of rows){
    const images=[];for(const image of row.images||[])images.push(await persist(image));
    const content=[];for(const block of row.content||[]){content.push(block?.type==='image'&&block.image?{...block,image:await persist(block.image)}:block)}
    out.push({...row,images,content});
  }
  return out;
}
function notionGuideRows(meta={},source={}){
  ...
  for(const page of Array.isArray(meta.sections)?meta.sections:[]){
    ...
    for(const [index,block] of (Array.isArray(page?.blocks)?page.blocks:[]).entries()){
      ...
      const body=String(block?.text||'').trim().slice(0,3000);
      const images=(Array.isArray(block?.images)?block.images:[]).slice(0,12);
      if(!title&&!body&&!images.length)continue;
      const key=(title+'\n'+body+'\n'+images.map(row=>row.src||'').join('|')).toLowerCase();
      if(seen.has(key))continue;seen.add(key);
      out.push({
        id:['notion',sourceId||'source',pageId||'page',index].join('-').replace(/[^a-zA-Z0-9가-힣-]+/g,'-').slice(0,120),
        sourceId,pageId,pageTitle,title:title||'본문',text:body,images,content:Array.isArray(block?.content)?block.content:[],depth
      });
  ...
  if(!out.length&&meta.contentText){
    out.push({
      ...
      title:String(meta.title||'Notion 가이드'),text:String(meta.contentText||'').slice(0,3000),depth:0
    });
  }
  return out;
}
  ...
  for(const item of rows){
    ...
    for(const source of sources){
      try{
        ...
        const guideRows=await persistNotionGuideMedia(notionGuideRows(meta,source),item.id);
        next.push(...guideRows);successCount++;
        details.push({itemId:item.id,sourceId:source.id||'',title:meta.title||source.label||'',sectionCount:guideRows.length,pageCount:Number(meta.pageCount||0)});
      }catch(error){
...
async function refreshReferenceGuides(rows=[]){
  const changedIds=new Set(),failures=[],details=[];
  for(const item of rows){
    const sources=(item.sources||[]).filter(source=>isNamuSourceUrl(source.url));
    if(!sources.length)continue;
    const curated=(item.knowledgeSections||[]).filter(row=>!String(row.id||'').startsWith('auto-namu-'));
    const auto=[];
    for(const source of sources){
      try{
        const meta=await fetchSourceMeta(source.url);
        for(const row of Array.isArray(meta.sections)?meta.sections:[]){
          auto.push({...row,sourceId:String(source.id||row.sourceId||'')});
        }
        details.push({itemId:item.id,sourceId:source.id||'',title:meta.title||source.label||'',sectionCount:(meta.sections||[]).length,imageCount:(meta.images||[]).length,strategy:meta.strategy||'namuwiki-structured-html'});
      }catch(error){
    ...
    const next=[...curated,...auto].slice(0,120);
    if(JSON.stringify(item.knowledgeSections||[])!==JSON.stringify(next)){
      item.knowledgeSections=next;item.knowledgeSyncedAt=new Date().toISOString();changedIds.add(item.id);
    }
  }
  return{rows,changedIds:[...changedIds],failures,details};
}
