...
const AUTO_GUIDE_MEDIA_MIGRATION_KEY='content-archive:auto-sync:guide-media-migration:v7-no-broken-namuwiki-assets';
...
function namuImageFromTag(tag='',pageUrl=''){
  ...
  // UI icons, platform logos and generic decorative assets add noise. Keep images that describe Leopel itself.
  if(filename&&!/레오펠|사자의 노래|신비의.?동상|거주지|전체.?지도|비석|한남더헬/i.test(filename))return null;
  try{
    const src=new URL(raw.startsWith('//')?'https:'+raw:raw,pageUrl).toString();
    return{
      src,alt:filename||'레오펠 자료 이미지',caption:filename||'레오펠 자료 이미지',
...
function namuReadTransportUrl(sourceUrl){
  const source=sourceUrl instanceof URL?sourceUrl:new URL(String(sourceUrl||''));
  if(!isNamuSourceUrl(source.toString()))return null;
  const transport=new URL('https://d.namu.moe');
  transport.pathname=source.pathname;
  transport.search=source.search;
  return transport;
}
async function fetchNamuSourceMeta(sourceUrl,{signal}={}){
  const source=sourceUrl instanceof URL?sourceUrl:new URL(String(sourceUrl||''));
  const read=async target=>{
    const response=await fetch(target,{redirect:'follow',signal,headers:SOURCE_BROWSER_HEADERS});
    const html=(await response.text()).slice(0,2000000);
    return{response,html};
  };
  const direct=await read(source);
  if(direct.response.ok)return fetchNamuMeta(source,direct.html);
  if(![403,429].includes(direct.response.status))throw new Error('source_meta_fetch_'+direct.response.status);
  const transport=namuReadTransportUrl(source);
  if(!transport)throw new Error('source_meta_fetch_'+direct.response.status);
  const fallback=await read(transport);
  if(!fallback.response.ok)throw new Error('source_meta_namu_transport_'+fallback.response.status);
  const contentType=String(fallback.response.headers.get('content-type')||'');
  if(!contentType.includes('text/html')&&!contentType.includes('application/xhtml+xml'))throw new Error('source_meta_not_html');
  const meta=fetchNamuMeta(source,fallback.html);
  return{...meta,strategy:'namuwiki-structured-fallback-transport'};
}
...
function namuGuideImageProxyUrl(media={}){
  const raw=String(media?.src||'').trim();if(!raw)return'';
  try{
    const url=new URL(raw),host=url.hostname.toLowerCase();
    if(!['file.namu.moe','d.namu.moe','namu.moe','www.namu.moe'].includes(host))return'';
    if(url.protocol!=='https:')return'';
    return '/api/content?type=namuwiki-guide-image&src='+encodeURIComponent(url.toString());
  }catch{return''}
}
async function handleNamuGuideImage(req,res){
  ...
  const requestUrl=new URL(req?.url||'/','https://archive.local'),raw=String(requestUrl.searchParams.get('src')||'');
  let source=null;
  try{
    source=new URL(raw);
    const host=source.hostname.toLowerCase();
    if(source.protocol!=='https:'||!['file.namu.moe','d.namu.moe','namu.moe','www.namu.moe'].includes(host))throw new Error('invalid_namuwiki_image_ref');
  }catch{
    if(typeof res.status==='function')return res.status(400).json({error:'invalid_namuwiki_image_ref'});
    res.statusCode=400;return res.end?.('invalid_namuwiki_image_ref');
  }
  try{
    const response=await fetch(source.toString(),{
      redirect:'follow',
      headers:{...SOURCE_BROWSER_HEADERS,Accept:'image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8',Referer:'https://namu.wiki/'}
    });
    if(!response.ok)throw new Error('namuwiki_image_fetch_'+response.status);
    const contentType=String(response.headers.get('content-type')||'').split(';')[0].trim().toLowerCase();
    if(!contentType.startsWith('image/'))throw new Error('namuwiki_image_not_image');
    const contentLength=Number(response.headers.get('content-length')||0);
    if(contentLength>12*1024*1024)throw new Error('namuwiki_image_too_large');
    const bytes=Buffer.from(await response.arrayBuffer());
    if(!bytes.length||bytes.length>12*1024*1024)throw new Error('namuwiki_image_too_large');
    res.setHeader?.('Content-Type',contentType);
    res.setHeader?.('Content-Length',String(bytes.length));
    res.setHeader?.('Cache-Control','public, max-age=86400, s-maxage=604800, stale-while-revalidate=2592000');
    res.setHeader?.('CDN-Cache-Control','public, max-age=604800, stale-while-revalidate=2592000');
    ...
    if(typeof res.status==='function')return res.status(502).json({error:String(error?.message||'namuwiki_image_unavailable')});
    res.statusCode=502;return res.end?.('namuwiki_image_unavailable');
  }
}
...
async function sourceImageAvailable(raw='',{referer='https://namu.wiki/'}={}){
  const src=String(raw||'');if(!/^https:\/\//i.test(src))return false;
  try{
    const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),5000);
    try{
      const response=await fetch(src,{method:'GET',redirect:'follow',signal:controller.signal,headers:{...SOURCE_BROWSER_HEADERS,Accept:'image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8',Referer:referer}});
...
async function persistReferenceImage(media={},itemId='content'){
  ...
  if(persisted?.permanent===true||/res\.cloudinary\.com/i.test(String(persisted?.src||'')))return persisted;
  const proxy=namuGuideImageProxyUrl(media);
  if(!proxy)return persisted;
  const available=await sourceImageAvailable(media.src,{referer:'https://namu.wiki/'});
  return available?{...media,src:proxy,assetState:'proxy',permanent:false}:{...media,src:'',assetState:'unavailable',permanent:false};
}
...
async function handlePublicAutoSync(req,res){
  ...
  const requestUrl=new URL(req?.url||'/','https://archive.local');
  if(method==='GET'&&requestUrl.searchParams.get('migration')==='guide-media-v7'){
    if(!hasRedis())return json(res,503,{error:'archive_storage_unavailable'});
    const marker=String(await redisCommand('GET',AUTO_GUIDE_MEDIA_MIGRATION_KEY).catch(()=>''));
    if(marker==='done')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_v7_complete',guideMediaMigration:true});
    const lockKey=AUTO_LOCK_KEY+':guide-media-v7',lockToken=Date.now()+'-'+Math.random().toString(36).slice(2);
    const locked=await redisCommand('SET',lockKey,lockToken,'NX','EX',180).catch(()=>null);
    if(locked!=='OK')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_v7_locked',guideMediaMigration:true});
    try{
