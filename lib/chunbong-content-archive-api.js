...
const AUTO_FULL_MIGRATION_KEY='content-archive:auto-sync:full-migration:v4-bngts-hidden-ingestion';
...
async function refreshInternalReferenceSources(rows=[]){
  ...
  for(const row of fetched){
    ...
    details.push({itemId:row.item.id,sourceId:row.source.id||'',participantCount:Number(meta.participantCount||0),strategy:meta.strategy||'html-meta'});
  }
  ...
  for(const page of Array.isArray(meta.sections)?meta.sections:[]){
    ...
    for(const [index,block] of (Array.isArray(page?.blocks)?page.blocks:[]).entries()){
      ...
      out.push({
        ...
        sourceId,pageId,pageTitle,title:title||'본문',text:body,depth
      });
