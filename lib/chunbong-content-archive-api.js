...
async function archiveGuideUploadFile(media={}){
  ...
  try{
    const headers={...SOURCE_BROWSER_HEADERS,Accept:'image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8'};
    if(/(?:^|\.)namu\.(?:wiki|moe)$|(?:^|\.)namu\.moe$/.test(parsed.hostname)||parsed.hostname==='file.namu.moe')headers.Referer='https://namu.wiki/';
    if(/notion|amazonaws\.com$|notionusercontent\.com$/.test(parsed.hostname))headers.Referer='https://www.notion.so/';
...
async function persistNotionImage(media={},itemId='content'){
  return persistArchiveGuideImage(media,itemId,'notion');
}
async function persistReferenceImage(media={},itemId='content'){
  return persistArchiveGuideImage(media,itemId,'reference');
}
async function persistGuideMedia(rows=[],itemId='content',persistImage=persistNotionImage){
  ...
  const persist=async media=>{
    ...
    const key=String(media.originalId||media.originUrl||media.sourceUrl||media.src);
    if(!cache.has(key))cache.set(key,persistImage(media,itemId));
...
async function refreshReferenceGuides(rows=[]){
  ...
  for(const item of rows){
    ...
    for(const source of sources){
      try{
        const meta=await fetchSourceMeta(source.url);
        const guideRows=await persistReferenceGuideMedia(referenceGuideRows(meta,source,'namuwiki'),item.id);
        next.push(...guideRows);successCount++;
        details.push({
          itemId:item.id,sourceId:source.id||'',title:meta.title||source.label||'',
          sectionCount:guideRows.length,imageCount:guideRows.flatMap(row=>row.images||[]).length,
...
module.exports={
  ...
  _internals:{publicRows,promoteOfficialHero,syntheticArchiveVisual,mergeArchiveRows,mergeCollections,mergeCuratedMaterial,applyCuratedVisibility,allowedSourceMetaUrl,fetchSourceMeta,normalizePublishedDate,readJsonLdPublishedDate,readPublishedDate,extractNotionPageId,notionSignedUrl,notionPageRows,fetchNotionTree,notionStructuredSections,notionGuideRows,cloudinaryArchiveConfig,cloudinaryArchiveKey,persistArchiveGuideImage,persistNotionImage,persistReferenceImage,persistGuideMedia,persistNotionGuideMedia,persistReferenceGuideMedia,refreshNotionGuides,isNotionSourceUrl,isNamuSourceUrl,extractNamuStructured,fetchNamuMeta,namuReadTransportUrl,fetchNamuSourceMeta,refreshReferenceGuides,isBngtsSourceUrl,applyBngtsParticipantSnapshot,refreshInternalReferenceSources,extractBngtsStreamerNames,extractSoopPostRef,parseSoopPostPayload,withoutHidden,curatedHiddenIds,storedRows,adminRows,autoSyncState,autoSyncOfficialArchive,prepareForSave,redisCommand,hasRedis,INDEX_KEY,ITEM_PREFIX,DRAFT_INDEX,DRAFT_PREFIX,HIDDEN_KEY,AUTO_LAST_KEY,AUTO_CANDIDATES_KEY,BROWSER_IMPORT_PREFIX,BROWSER_IMPORT_INDEX,normalizeBrowserImportPayload,applyBrowserImportToItem,handleOperatorCandidate,handleOperatorBrowserImport,shouldForcePublicAutoSync,explicitSameOrigin,AUTO_FULL_MIGRATION_KEY}
};
