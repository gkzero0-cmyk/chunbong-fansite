...
async function fetchNotionMeta(url,html,{signal}={}){
  ...
  const payload=await fetchNotionChunk(pageId,{signal});
  const rows=notionPageRows(payload.recordMap,pageId);
  const root=rows[0]||{};
  const outline=rows.filter(row=>row.text&&['page','header','sub_header','sub_sub_header'].includes(row.type)).map(row=>row.text).slice(0,120);
  const bodyText=rows.filter(row=>row.text&&row.id!==pageId).map(row=>row.text).join('\n').slice(0,12000);
  return{
    url:url.toString(),title:String(root.text||readTitle(html)||'').slice(0,200),
    description:bodyText.slice(0,500),image:'',pageId,
    outline,contentText:bodyText,strategy:'notion-record-map'
  };
}
...
function isNotionHost(host=''){return host==='notion.site'||host.endsWith('.notion.site')||host==='www.notion.so'||host==='notion.so'}
function isFmkoreaHost(host=''){return host==='fmkorea.com'||host==='www.fmkorea.com'}
...
async function fetchSourceMeta(rawUrl){
  ...
  try{
    ...
    const html=(await response.text()).slice(0,2000000);
    if(isFmkoreaHost(url.hostname)&&(response.status===430||/에펨코리아 보안 시스템|challenges\.cloudflare\.com\/turnstile/i.test(html)))throw new Error('source_meta_human_verification_required');
    if(!response.ok)throw new Error('source_meta_fetch_'+response.status);
    ...
    if(isSoopStationPost(finalUrl)&&!title&&/(?:\/_next\/static\/|__next)/i.test(html))throw new Error('source_meta_client_render_required');
