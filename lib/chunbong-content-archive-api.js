...
async function fetchNotionMeta(url,html,{signal}={}){
  ...
  return{
    ...
    outline:outline.slice(0,300),contentText:bodyText,pageCount:pages.length,strategy:'notion-record-map-recursive'
  };
}
...
async function autoSyncOfficialArchive({force=false}={}){
  ...
  const lastMs=Date.parse(state.last?.completedAt||'');
  ...
  try{
    const [rows,discovery]=await Promise.all([adminRows(),autoIngest.discoverOfficialArchiveMaterials()]);
    ...
    const summary={
      completedAt,sourcePriority:['SOOP','YouTube'],discovered:discovery.counts,
...
async function handlePublicAutoSync(req,res){
  ...
  if(method==='POST'){
    ...
    if(!/vercel-cron/i.test(userAgent))return json(res,403,{error:'cron_only'});
...
module.exports={
  handlePublicList,handlePublicDetail,handlePublicAutoSync,handleOperatorList,handleOperatorSave,handleOperatorPublish,handleOperatorAutoSync,handleOperatorSourceMeta,handleOperatorDelete,
