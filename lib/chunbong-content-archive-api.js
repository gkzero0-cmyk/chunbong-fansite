...
function extractNamuStructured(html='',sourceUrl=''){
  ...
  for(const [index,range] of ranges.entries()){
    ...
    const text=htmlPlainText(segment).replace(/최근 수정 시각\s*:\s*[^\n]+/g,'').trim().slice(0,6000);
...
async function archiveGuideUploadFile(media={}){
  ...
  const needsBrowserFetch=parsed.hostname==='file.namu.moe'||parsed.hostname==='d.namu.moe'||parsed.hostname==='namu.moe'||parsed.hostname.endsWith('.namu.moe');
  if(!needsBrowserFetch)return src;
  try{
    ...
    const filename=String(media.filename||media.alt||'reference-image').replace(/[\\/:*?"<>|]+/g,'-').slice(0,120)||'reference-image';
