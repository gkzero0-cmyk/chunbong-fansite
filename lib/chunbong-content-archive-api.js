...
function mergeArchiveRows(seedRows=[],storedRows=[]){
  ...
  const seeds=new Map((Array.isArray(seedRows)?seedRows:[]).map(raw=>[String(raw?.id||''),raw]));
  ...
  for(const raw of Array.isArray(storedRows)?storedRows:[]){
    ...
    const base=seeds.get(id)||{};
    const migrated={...base,...raw};
    for(const key of ['archiveType','parentId','editionLabel','editionOrder','featured']){
      if(raw?.[key]===undefined&&base?.[key]!==undefined)migrated[key]=base[key];
    }
    const item=normalizeArchiveItem(migrated);
    if(item.id)merged.set(item.id,item);
  }
