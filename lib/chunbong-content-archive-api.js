...
const AUTO_GUIDE_MEDIA_MIGRATION_KEY='content-archive:auto-sync:guide-media-migration:v2-all-public-guide-assets';
...
async function persistNotionImage(media={},itemId='content'){
  ...
  if(ephemeralNotionAssetUrl(media?.src))return {...media,src:'',assetState:'remote',permanent:false};
  return persisted;
}
...
async function handlePublicAutoSync(req,res){
  ...
  if(method==='GET'&&requestUrl.searchParams.get('migration')==='guide-media-v2'){
    if(!hasRedis())return json(res,503,{error:'archive_storage_unavailable'});
    const marker=String(await redisCommand('GET',AUTO_GUIDE_MEDIA_MIGRATION_KEY).catch(()=>''));
    if(marker==='done')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_complete',guideMediaMigration:true});
    const lockKey=AUTO_LOCK_KEY+':guide-media-v2',lockToken=Date.now()+'-'+Math.random().toString(36).slice(2);
    const locked=await redisCommand('SET',lockKey,lockToken,'NX','EX',180).catch(()=>null);
    if(locked!=='OK')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_locked',guideMediaMigration:true});
    try{
