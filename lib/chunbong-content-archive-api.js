...
function notionStructuredSections(pages=[]){
  ...
  for(const page of pages){
    ...
    const root=rows[0]||{},blocks=[];let current={title:String(root.text||'').trim()||'본문',lines:[],images:[]};
    const push=()=>{
    ...
    for(const row of rows.slice(1)){
      if(['header','sub_header','sub_sub_header'].includes(row.type)&&row?.text){
        push();current={title:String(row.text).trim(),lines:[],images:[]};continue;
      }
      if(row?.media){
      ...
      if(row?.text)current.lines.push(String(row.text).trim());
    }
    push();
  ...
  for(const page of Array.isArray(meta.sections)?meta.sections:[]){
    ...
    for(const [index,block] of (Array.isArray(page?.blocks)?page.blocks:[]).entries()){
      ...
      out.push({
        ...
        sourceId,pageId,pageTitle,title:title||'본문',text:body,images,depth
      });
