...
function notionBlockMedia(block={}){
  ...
  return{src,sourceUrl:src,caption,filename,alt:caption||filename||'Notion 이미지'};
}
...
function htmlPlainText(value=''){
  return decodeHtml(String(value||'').replace(/<script[\\s\\S]*?<\\/script>/gi,' ').replace(/<style[\\s\\S]*?<\\/style>/gi,' ').replace(/<br\\s*\\/?>/gi,'\\n').replace(/<\\/p>/gi,'\\n').replace(/<[^>]+>/g,' ').replace(/\\s+/g,' ').trim());
}
  ...
  for(const item of rows){
    ...
    for(const source of sources){
      try{
        ...
        const guideRows=notionGuideRows(meta,source);
