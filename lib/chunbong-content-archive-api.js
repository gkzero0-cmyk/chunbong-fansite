...
const AUTO_GUIDE_MEDIA_MIGRATION_KEY='content-archive:auto-sync:guide-media-migration:v8-fast-safe-guide-assets';
...
async function persistReferenceImage(media={},itemId='content'){
  ...
  if(persisted?.permanent===true||/res\.cloudinary\.com/i.test(String(persisted?.src||'')))return persisted;
  // file.namu.moe can return 503 for long periods. Never keep a public image
  // URL that we already failed to archive; dropping it is safer than rendering
  // a broken card and keeps the rest of the archive sync fast.
  if(namuGuideImageProxyUrl(media))return{...media,src:'',assetState:'unavailable',permanent:false};
  return persisted;
}
...
async function handlePublicAutoSync(req,res){
  ...
  const requestUrl=new URL(req?.url||'/','https://archive.local');
  if(method==='GET'&&requestUrl.searchParams.get('migration')==='guide-media-v8'){
    if(!hasRedis())return json(res,503,{error:'archive_storage_unavailable'});
    const marker=String(await redisCommand('GET',AUTO_GUIDE_MEDIA_MIGRATION_KEY).catch(()=>''));
    if(marker==='done')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_v8_complete',guideMediaMigration:true});
    const lockKey=AUTO_LOCK_KEY+':guide-media-v8',lockToken=Date.now()+'-'+Math.random().toString(36).slice(2);
    const locked=await redisCommand('SET',lockKey,lockToken,'NX','EX',180).catch(()=>null);
    if(locked!=='OK')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_v8_locked',guideMediaMigration:true});
    try{
