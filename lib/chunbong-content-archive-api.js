...
function mergeArchiveRows(seedRows=[],storedRows=[]){
  ...
  for(const raw of (Array.isArray(storedRows)?storedRows:[])){
    ...
    if(seeded){
      ...
      if(seeded.heroImage?.src&&(!item.heroImage?.src||(/res\.cloudinary\.com/.test(seeded.heroImage.src)&&!/res\.cloudinary\.com/.test(item.heroImage.src))))item.heroImage=seeded.heroImage;
    }
