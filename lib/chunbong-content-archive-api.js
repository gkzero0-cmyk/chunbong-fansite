...
async function handleOperatorList(req,res){
  ...
  try{
    ...
    const [auto,browserImports]=await Promise.all([autoSyncState(),browserImportInboxRows(rows)]);
    return json(res,200,{items:rows,autoSync:auto.last,candidates:auto.candidates,browserImports});
...
function browserImportSource(payload){
  if(payload.source==='fmkorea-public-browser'){
    return{id:'source-fmk-'+payload.postId,kind:'reference',label:'FM코리아 공개 게시글 · '+(payload.title||payload.postId),url:payload.url,visibility:'public'};
  }
...
function browserImportTimeline(payload){
  if(payload.source==='fmkorea-public-browser'){
    ...
    return{
      ...
      note:'FM코리아 공개 게시글에서 자동 수집한 참고자료입니다.'+(meta?' · '+meta:''),
      visibility:'public'
    };
  }
  if(payload.public){
...
function browserImportPublicEligible(payload={}){
  return payload?.source==='fmkorea-public-browser'||payload?.public===true||String(payload?.access||'')==='anonymous-verified';
}
...
async function browserImportInboxRows(rows=[]){
  ...
  return records.map(({recordId,record})=>{
    ...
    return{
      ...
      access:String(payload.access||''),publicEligible:browserImportPublicEligible(payload),state,
...
function applyBrowserImportWithVisibility(rawItem,payload,visibility='internal'){
  const publicAllowed=browserImportPublicEligible(payload);
  ...
  if(source){
    ...
    source.label=visibility==='public'?(payload.source==='fmkorea-public-browser'?'FM코리아 공개 게시글 · ':'SOOP 공개 게시글 · ')+(payload.title||payload.postId):source.label;
  }
  if(timeline){
    ...
    if(visibility==='public'){
      ...
      timeline.note=payload.source==='fmkorea-public-browser'?'FM코리아 공개 게시글에서 자동 수집한 참고자료입니다.':'SOOP에서 비로그인 상태로도 확인되는 공개 게시글입니다.';
    }else{
...
async function handleOperatorBrowserImportManage(req,res){
  ...
  try{
    ...
    if(visibility==='public'&&!browserImportPublicEligible(payload))return json(res,400,{error:'browser_import_not_publicly_accessible'});
    if(linked&&linked.id!==target.id){
      await saveArchiveItemDirect(detachBrowserImportFromItem(linked,payload));
    }
    const saved=await saveArchiveItemDirect(applyBrowserImportWithVisibility(target,payload,visibility));
    await updateBrowserImportManagement(recordId,{managementStatus:'',managedAction:action,linkedItemId:saved.id,visibility});
    return json(res,200,{ok:true,action,item:saved,visibility,publicEligible:browserImportPublicEligible(payload)});
  }catch(error){
