...
async function handlePublicAutoSync(req,res){
  ...
  // Public page visits must never start archive collection. Scheduled GitHub
  // Actions calls authenticate with OIDC; owner-triggered manual sync uses the
  // separate operator-content-auto-sync endpoint.
  const githubOidc=await pushNotifications._internals.authorizedGitHubOidc(req);
  if(!githubOidc)return json(res,403,{error:'auto_sync_unauthorized'});
  const migrationMarker=String(await redisCommand('GET',AUTO_FULL_MIGRATION_KEY).catch(()=>''));
  const guideMigrationMarker=String(await redisCommand('GET',AUTO_GUIDE_MEDIA_MIGRATION_KEY).catch(()=>''));
  const force=shouldForcePublicAutoSync({githubOidc:true,migrationMarker});
  const refreshGuideMedia=guideMigrationMarker!=='done'&&!force;
  try{
...
function normalizeBrowserImportPayload(raw={}){
  ...
  const rawAccess=String(raw.access||'').trim(),access=['anonymous-verified','favorite','subscriber','authenticated','private'].includes(rawAccess)?rawAccess:'authenticated';
  const isPublic=access==='anonymous-verified';
  return{
    version:1,source,platform:'soop',public:isPublic,access,postId:match[1],url:url.toString(),
...
function browserImportSource(payload){
  ...
  if(payload.access==='favorite')return{id:'source-soop-auth-'+payload.postId,kind:'official',label:'SOOP 애청자 게시글 · '+(payload.title||payload.postId),url:payload.url,visibility:'public'};
  return{id:'source-soop-auth-'+payload.postId,kind:'official',label:'SOOP 로그인 제한 확인 글 · '+(payload.title||payload.postId),url:payload.url,visibility:'internal'};
}
function browserImportTimeline(payload){
  ...
  if(payload.public||payload.access==='favorite'){
    const source=browserImportSource(payload),favorite=payload.access==='favorite';
    return{id:'soop-auth-post-'+payload.postId,type:'post',title:payload.title||((favorite?'SOOP 애청자 게시글 · ':'SOOP 공개 게시글 · ')+payload.postId),date:payload.date,datePrecision:payload.date?'day':'unknown',url:payload.url,thumbnail:payload.images?.[0]||'',sourceId:source.id,note:favorite?'SOOP 애청자 공개 게시글에서 자동 수집한 공식 자료입니다.':'SOOP에서 비로그인 상태로도 확인되는 공개 게시글입니다.',visibility:'public'};
  }
  const accessLabel=payload.access==='subscriber'?'구독자 전용':payload.access==='private'?'비공개':payload.access==='authenticated'?'로그인 제한':'권한 제한';
  return{
    id:'soop-auth-post-'+payload.postId,type:'post',title:payload.title||('SOOP '+accessLabel+' 글 · '+payload.postId),
    date:payload.date,datePrecision:payload.date?'day':'unknown',url:'',thumbnail:'',sourceId:'',
    note:'SOOP '+accessLabel+' 자료입니다. 운영자 확인 전에는 팬사이트 일반 방문자에게 노출하지 않습니다.',
    visibility:'internal'
  };
}
...
function browserImportPublicEligible(payload={}){
  ...
  return payload?.public===true||access==='anonymous-verified'||access==='favorite';
}
function browserImportNeedsReview(payload={}){
  if(payload?.source!=='soop-authenticated-browser')return false;
  return !browserImportPublicEligible(payload);
}
...
async function browserImportInboxRows(rows=[]){
  ...
  return records.map(({recordId,record})=>{
    ...
    return{
      ...
      access:String(payload.access||''),publicEligible:browserImportPublicEligible(payload),reviewRequired:browserImportNeedsReview(payload),operatorPublicAllowed:payload.source==='soop-authenticated-browser'&&browserImportNeedsReview(payload),state,
...
function applyBrowserImportWithVisibility(rawItem,payload,visibility='internal',{operatorOverride=false}={}){
  ...
  if(source){
    ...
    source.label=visibility==='public'?(payload.source==='fmkorea-public-browser'?'FM코리아 공개 게시글 · ':payload.access==='favorite'?'SOOP 애청자 게시글 · ':payload.public?'SOOP 공개 게시글 · ':'SOOP 로그인 제한 게시글 · ')+(payload.title||payload.postId):source.label;
  }
  if(timeline){
    ...
    if(visibility==='public'){
      ...
      timeline.note=payload.source==='fmkorea-public-browser'?'FM코리아 공개 게시글에서 자동 수집한 참고자료입니다.':payload.access==='favorite'?'SOOP 애청자 공개 게시글에서 자동 수집한 공식 자료입니다.':payload.public?'SOOP에서 비로그인 상태로도 확인되는 공개 게시글입니다.':'SOOP 로그인 제한 자료를 운영자 확인 후 공개 참고자료로 전환했습니다.';
    }else{
...
async function handleOperatorBrowserImport(req,res){
  ...
  try{
    ...
    const rows=await adminRows(),source=browserImportSource(payload),autoVisibility=browserImportPublicEligible(payload)?'public':'internal';
    const existing=rows.find(row=>(row.sources||[]).some(entry=>entry.id===source.id||entry.url===source.url));
    if(action==='auto'&&existing){
      item=await saveArchiveItemDirect(applyBrowserImportWithVisibility(existing,payload,autoVisibility));
      await updateBrowserImportManagement(browserImportRecordId(payload),{managementStatus:'',managedAction:autoVisibility==='public'?'auto-public':'auto-internal',linkedItemId:item.id,visibility:autoVisibility,autoMatchedAt:new Date().toISOString()});
      return json(res,200,{ok:true,action,matched:true,item,postId:payload.postId,storedRaw:true,duplicate:true,upgradedPublic:autoVisibility==='public',autoVisibility,source:payload.source});
    }
    if(action==='auto'){
      match=matchBrowserImport(payload,rows);
      if(!match)return json(res,200,{ok:true,action,matched:false,postId:payload.postId,storedRaw:true,reviewRequired:browserImportNeedsReview(payload),source:payload.source});
      const currentItem=rows.find(row=>row.id===match.itemId);
      if(!currentItem)return json(res,200,{ok:true,action,matched:false,postId:payload.postId,storedRaw:true,reviewRequired:browserImportNeedsReview(payload),source:payload.source});
      item=await saveArchiveItemDirect(applyBrowserImportWithVisibility(currentItem,payload,autoVisibility));
      await updateBrowserImportManagement(browserImportRecordId(payload),{managementStatus:'',managedAction:autoVisibility==='public'?'auto-public':'auto-internal',linkedItemId:item.id,visibility:autoVisibility,autoMatchedAt:new Date().toISOString(),matchScore:Number(match.score||0)});
      return json(res,200,{ok:true,action,matched:true,match,item,postId:payload.postId,storedRaw:true,autoVisibility,reviewRequired:browserImportNeedsReview(payload),source:payload.source});
    }
    if(action==='connect'){
      const currentItem=rows.find(row=>row.id===String(body.itemId||''));
      if(!currentItem)return json(res,404,{error:'content_not_found'});
      item=await saveArchiveItemDirect(applyBrowserImportWithVisibility(currentItem,payload,autoVisibility));
      await updateBrowserImportManagement(browserImportRecordId(payload),{managementStatus:'',managedAction:'connect',linkedItemId:item.id,visibility:autoVisibility});
    }else{
      ...
      item=await saveArchiveItemDirect(normalizeArchiveItem({
        ...
        summary:isFmk?'FM코리아 공개 게시글에서 수집한 새 콘텐츠 후보입니다.':payload.access==='favorite'?'SOOP 애청자 게시글에서 수집한 새 콘텐츠 후보입니다.':'SOOP 로그인 제한 자료에서 수집한 내부 검토용 새 콘텐츠 후보입니다.',
        description:isFmk?'공개 원문 링크를 기준으로 일정·참가자·규칙·결과를 구조화해 주세요.':payload.access==='favorite'?'애청자 공개 자료를 기준으로 콘텐츠 정보를 구조화해 주세요.':'구독·비공개·기타 로그인 제한 원문은 내부 검증 자료로 보관하고 공개 전 운영자가 확인해 주세요.',
