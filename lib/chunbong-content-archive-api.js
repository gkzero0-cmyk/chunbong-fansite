...
async function storedRows(){
  ...
  try{
    const stored=await readIndexedRows(INDEX_KEY,ITEM_PREFIX);
    return publicRows(mergeArchiveRows(seed.items||[],stored));
...
async function adminRows(){
  ...
  return mergeArchiveRows(seed.items||[],published.concat(drafts));
}
...
module.exports={
  ...
  _internals:{publicRows,mergeArchiveRows,storedRows,adminRows,prepareForSave,redisCommand,hasRedis,INDEX_KEY,ITEM_PREFIX,DRAFT_INDEX,DRAFT_PREFIX}
};
