...
function normalizeBrowserImportPayload(raw={}){
  ...
  return{
    version:1,source,platform:'soop',public:false,postId:match[1],url:url.toString(),
...
function browserImportTimeline(payload){
  ...
  return{
    ...
    note:'SOOP 로그인 브라우저에서 확인한 내부 검증 기록입니다. 운영자가 공개 범위를 직접 변경하기 전에는 팬사이트에 노출하지 않습니다.',
    visibility:'internal'
  };
}
...
async function saveBrowserImportRecord(payload){
  const record={...payload,storedAt:new Date().toISOString()},recordId=browserImportRecordId(payload);
  await Promise.all([
