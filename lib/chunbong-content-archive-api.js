...
function mergeArchiveRows(seedRows=[],storedRows=[]){
  ...
  for(const raw of (Array.isArray(storedRows)?storedRows:[])){
    ...
    if(seeded){
      ...
      item.gallery=mergeCollections(seeded.gallery,item.gallery,(seedRow,storedRow)=>({...storedRow,...seedRow}));
      ...
      if(seededHero&&(!storedHero||(syntheticArchiveVisual(storedHero)&&!syntheticArchiveVisual(seededHero))))item.heroImage=seeded.heroImage;
      else if(seededHero&&/res\.cloudinary\.com/.test(seededHero)&&!/res\.cloudinary\.com/.test(storedHero))item.heroImage=seeded.heroImage;
    }
