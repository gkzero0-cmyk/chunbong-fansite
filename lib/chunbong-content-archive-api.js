...
async function fetchSourceMeta(rawUrl){
  ...
  try{
    ...
    return{url:finalUrl.toString(),title:readTitle(html).slice(0,200),description:description.slice(0,500),image,publishedDate};
...
module.exports={
  ...
  _internals:{publicRows,mergeArchiveRows,applyCuratedVisibility,allowedSourceMetaUrl,fetchSourceMeta,normalizePublishedDate,readJsonLdPublishedDate,readPublishedDate,withoutHidden,curatedHiddenIds,storedRows,adminRows,prepareForSave,redisCommand,hasRedis,INDEX_KEY,ITEM_PREFIX,DRAFT_INDEX,DRAFT_PREFIX,HIDDEN_KEY}
};
