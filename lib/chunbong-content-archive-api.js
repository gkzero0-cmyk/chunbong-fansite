...
async function autoSyncOfficialArchive({force=false}={}){
  ...
  try{
    ...
    const internalReferences=await refreshInternalReferenceSources(notion.rows);
    const changed=new Set([...applied.changedIds,...notion.changedIds,...internalReferences.changedIds]);
    ...
    const summary={
      ...
      sourcePriority:['SOOP','YouTube','Notion'],discovered:discovery.counts,scanMode:discovery.scanMode||'incremental',
    ...
    const failure={
      ...
      sourcePriority:['SOOP','YouTube','Notion']
    };
