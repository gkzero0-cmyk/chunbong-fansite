...
function mergeArchiveRows(seedRows=[],storedRows=[]){
  ...
  for(const raw of (Array.isArray(storedRows)?storedRows:[])){
    ...
    if(seeded?.sources?.length&&item.sources?.length){
      const seededSources=new Map(seeded.sources.map(row=>[row.id||row.url,row]));
      item.sources=item.sources.map(row=>{
        const curated=seededSources.get(row.id||row.url);
        return curated?.visibility==='internal'?{...row,visibility:'internal'}:row;
      });
    }
...
module.exports={
  ...
  _internals:{publicRows,mergeArchiveRows,withoutHidden,curatedHiddenIds,storedRows,adminRows,prepareForSave,redisCommand,hasRedis,INDEX_KEY,ITEM_PREFIX,DRAFT_INDEX,DRAFT_PREFIX,HIDDEN_KEY}
};
