...
const AUTO_GUIDE_MEDIA_MIGRATION_KEY='content-archive:auto-sync:guide-media-migration:v6-permanent-and-reference-proxy';
...
async function archiveGuideUploadFile(media={}){
  ...
  let fetchUrl=src;
  if(media?.attachmentSource&&media?.originalId){
    try{
      fetchUrl=await notionSignedImageUrl({
        block:String(media.originalId||''),source:String(media.attachmentSource||''),
        space:String(media.spaceId||''),table:String(media.parentTable||'block')
      });
    }catch{}
  }
  let parsed=null;try{parsed=new URL(fetchUrl)}catch{return src}
  if(!/^https:$/.test(parsed.protocol))return src;
  try{
    const isNamuHost=parsed.hostname==='file.namu.moe'||parsed.hostname==='namu.moe'||parsed.hostname.endsWith('.namu.moe')||parsed.hostname==='namu.wiki'||parsed.hostname.endsWith('.namu.wiki');
    const isNotionHost=/notion|amazonaws\.com$|notionusercontent\.com$/.test(parsed.hostname);
    const headers={...SOURCE_BROWSER_HEADERS,Accept:'image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8',...(isNamuHost?{Referer:'https://namu.wiki/'}:isNotionHost?{Referer:'https://www.notion.so/'}:{})};
    const response=await fetch(fetchUrl,{redirect:'follow',headers});
...
function referenceGuideImageProxyUrl(media={}){
  const raw=String(media?.src||'').trim();if(!raw)return'';
  try{
    const url=new URL(raw),host=url.hostname.toLowerCase();
    const allowed=host==='file.namu.moe'||host==='d.namu.moe'||host==='namu.moe'||host.endsWith('.namu.moe');
    if(!allowed||url.protocol!=='https:')return'';
    return'/api/content?type=reference-guide-image&url='+encodeURIComponent(url.toString());
  }catch{return''}
}
function allowedReferenceGuideImageUrl(value=''){
  try{
    const url=new URL(String(value||'')),host=url.hostname.toLowerCase();
    if(url.protocol!=='https:')return null;
    return host==='file.namu.moe'||host==='d.namu.moe'||host==='namu.moe'||host.endsWith('.namu.moe')?url:null;
  }catch{return null}
}
async function handleReferenceGuideImage(req,res){
  if(String(req?.method||'GET').toUpperCase()!=='GET'){
    if(typeof res.status==='function')return res.status(405).json({error:'method_not_allowed'});
    res.statusCode=405;return res.end?.('method_not_allowed');
  }
  const requestUrl=new URL(req?.url||'/','https://archive.local');
  const target=allowedReferenceGuideImageUrl(requestUrl.searchParams.get('url')||'');
  if(!target||target.toString().length>1800){
    if(typeof res.status==='function')return res.status(400).json({error:'invalid_reference_image_ref'});
    res.statusCode=400;return res.end?.('invalid_reference_image_ref');
  }
  try{
    ...
    const contentType=String(response.headers.get('content-type')||'').split(';')[0].trim().toLowerCase();
    if(!contentType.startsWith('image/'))throw new Error('reference_image_not_image');
    const contentLength=Number(response.headers.get('content-length')||0);
    if(contentLength>12*1024*1024)throw new Error('reference_image_too_large');
    const bytes=Buffer.from(await response.arrayBuffer());
    if(!bytes.length||bytes.length>12*1024*1024)throw new Error('reference_image_too_large');
    res.setHeader?.('Content-Type',contentType);
    res.setHeader?.('Content-Length',String(bytes.length));
    res.setHeader?.('Cache-Control','public, max-age=86400, s-maxage=604800, stale-while-revalidate=2592000');
    res.setHeader?.('CDN-Cache-Control','public, max-age=604800, stale-while-revalidate=2592000');
    ...
    if(typeof res.status==='function')return res.status(502).json({error:String(error?.message||'reference_image_unavailable')});
    res.statusCode=502;return res.end?.('reference_image_unavailable');
  }
}
async function persistReferenceImage(media={},itemId='content'){
  ...
  if(persisted?.permanent===true||/res\.cloudinary\.com/i.test(String(persisted?.src||'')))return persisted;
  const proxy=referenceGuideImageProxyUrl(media);
  return proxy?{...media,src:proxy,assetState:'proxy',permanent:false}:persisted;
}
...
async function handlePublicAutoSync(req,res){
  ...
  const requestUrl=new URL(req?.url||'/','https://archive.local');
  if(method==='GET'&&requestUrl.searchParams.get('migration')==='guide-media-v6'){
    if(!hasRedis())return json(res,503,{error:'archive_storage_unavailable'});
    const marker=String(await redisCommand('GET',AUTO_GUIDE_MEDIA_MIGRATION_KEY).catch(()=>''));
    if(marker==='done')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_v6_complete',guideMediaMigration:true});
    const lockKey=AUTO_LOCK_KEY+':guide-media-v6',lockToken=Date.now()+'-'+Math.random().toString(36).slice(2);
    const locked=await redisCommand('SET',lockKey,lockToken,'NX','EX',180).catch(()=>null);
    if(locked!=='OK')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_v6_locked',guideMediaMigration:true});
    try{
