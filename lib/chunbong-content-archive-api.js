...
const AUTO_GUIDE_MEDIA_MIGRATION_KEY='content-archive:auto-sync:guide-media-migration:v9-canonical-namuwiki-browser';
...
async function handlePublicAutoSync(req,res){
  ...
  const requestUrl=new URL(req?.url||'/','https://archive.local');
  if(method==='GET'&&requestUrl.searchParams.get('migration')==='guide-media-v9'){
    if(!hasRedis())return json(res,503,{error:'archive_storage_unavailable'});
    const marker=String(await redisCommand('GET',AUTO_GUIDE_MEDIA_MIGRATION_KEY).catch(()=>''));
    if(marker==='done')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_v9_complete',guideMediaMigration:true});
    const lockKey=AUTO_LOCK_KEY+':guide-media-v9',lockToken=Date.now()+'-'+Math.random().toString(36).slice(2);
    const locked=await redisCommand('SET',lockKey,lockToken,'NX','EX',180).catch(()=>null);
    if(locked!=='OK')return json(res,200,{ok:true,skipped:true,reason:'guide_media_migration_v9_locked',guideMediaMigration:true});
    try{
...
function applyNamuBrowserImportToItem(rawItem,payload,source={}){
  ...
  const rows=(payload.sections||[]).map((row,index)=>({
