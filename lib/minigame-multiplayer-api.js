...
async function withLock(code,fn){
  ...
  for(let attempt=0;attempt<12;attempt+=1){
    ...
    if(attempt<11){
      const waitMs=Math.min(120,25+attempt*15);
      await new Promise(resolve=>setTimeout(resolve,waitMs));
    }
  }
  const error=new Error('room_busy');error.code='room_busy';error.retryAfterMs=180;throw error;
}
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(error?.code==='room_busy'){
      setHeader(res,'Retry-After','1');
      return sendJson(res,409,{error:'room_busy',retryAfterMs:Number(error.retryAfterMs)||180});
    }
    console.error('[minigame-multiplayer] unavailable:',error?.message||'unknown');
    return sendJson(res,503,{error:error?.code||'multiplayer_unavailable'});
  }
};
