...
const CHUNTRIS_MODES = Object.freeze(new Set(['classic','sprint40','hard']));
...
function normalizeRoomMode(game,value){
  ...
  const mode=String(value||'sprint40');
  return CHUNTRIS_MODES.has(mode)?mode:null;
}
...
async function createRoom(game,nickname,mode=GAME_MODES[game]){
  for(let attempt=0;attempt<12;attempt+=1){
    const code=randomCode();
    const token=randomToken();
    const now=new Date().toISOString();
    const room={
      version:1,code,game,mode,seed:randomSeed(),round:1,state:'waiting',startAt:null,winnerId:null,
...
function safeProgress(body,game,mode){
  ...
  if(game==='chuntris'&&mode!=='sprint40'&&status==='completed')status='playing';
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(action==='create'){
      ...
      const created=await createRoom(game,nickname,mode);
...
module.exports._internals={normalizeCode,normalizeNickname,normalizeGame,normalizeRoomMode,publicRoom,safeProgress,refreshRoomState,ROOM_TTL_SECONDS};
