...
function genericTitle(value=''){
  const raw=text(value);
  if(!raw)return true;
  return /(?:다시보기|vod|catch|clip|쇼츠|shorts?)\s*\d*$/i.test(raw)
    ||/^(?:춘봉 영상|춘봉 catch|춘봉 클립|춘봉tv 동영상|춘봉tv shorts)$/i.test(raw)
    ||/(?:관련\s*)?(?:youtube|유튜브)\s*(?:영상\s*)?\d+$/i.test(raw)
    ||/공식\s*게시글(?:\s*[·:#-]?\s*\d+)?$/i.test(raw);
}
...
function canonicalMaterialUrl(row={}){
  const raw=String(row.url||'').trim();
  if(!raw)return'';
  try{
    const url=new URL(raw),host=url.hostname.replace(/^www\./,'').toLowerCase();
    if(host==='youtube.com'||host==='m.youtube.com'||host==='youtu.be'){
      let id='';
      if(host==='youtu.be')id=url.pathname.split('/').filter(Boolean)[0]||'';
      else if(url.pathname==='/watch')id=url.searchParams.get('v')||'';
      else{id=(url.pathname.match(/^\/(?:shorts|embed)\/([^/?#]+)/)||[])[1]||'';}
      if(id)return'youtube:'+id;
    }
    if(host==='sooplive.com'){
      const post=url.pathname.match(/\/post\/(\d+)/i);if(post)return'soop-post:'+post[1];
    }
    if(host==='vod.sooplive.com'){
      const player=url.pathname.match(/\/player\/(\d+)/i);
      if(player)return'soop-'+String(row.type||'media').toLowerCase()+':'+player[1];
    }
    url.hash='';
...
function materialIdentity(row={}){
  const normalized=canonicalMaterialUrl(row);
...
module.exports={
  ...
  attachOfficialDiscoveries,discoverOfficialArchiveMaterials,mergeCandidates,materialFromVideo,materialFromYoutube,materialFromPost,genericTitle,
  canonicalMaterialUrl,materialIdentity
};
