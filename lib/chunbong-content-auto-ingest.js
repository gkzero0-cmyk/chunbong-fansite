...
async function fetchAllYoutubeOfficial(){
  const [videos,shorts]=await Promise.all([
    fetchYoutube.fetchAllChannelItems('videos',{maxPages:YOUTUBE_MAX_PAGES}).catch(()=>[]),
    fetchYoutube.fetchAllChannelItems('shorts',{maxPages:YOUTUBE_MAX_PAGES}).catch(()=>[])
  ]);
...
async function discoverOfficialArchiveMaterials(){
  const [vods,catches,clips,youtube,posts]=await Promise.all([
    fetchPagedSoopVideos('vod').catch(()=>[]),
    fetchPagedSoopVideos('catch').catch(()=>[]),
    fetchPagedSoopVideos('clip').catch(()=>[]),
    fetchAllYoutubeOfficial().catch(()=>({videos:[],shorts:[]})),
    fetchPagedSoopPosts().catch(()=>[])
  ]);
  const materials=[
  ...
  return{
    ...
    scope:{soop:'full-channel-history',youtube:'full-channel-history'}
  };
}
