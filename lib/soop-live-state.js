...
const DEFAULT_BROAD_FALLBACK_URL = 'https://api-channel.sooplive.com/v1.1/channel/chunbongtv/home/section/broad';
...
async function fetchSoopStructuredLive(options = {}) {
  ...
  const urls = options.url
    ? [options.url]
    : [DEFAULT_BROAD_URL, DEFAULT_BROAD_FALLBACK_URL];
  const signals = [];
  const errors = [];
  ...
  for (const url of urls) {
    try {
      ...
      const state = normalizeSoopBroadPayload(await response.json(), url.includes('.com/') ? 'soop-channel-com' : 'soop-channel');
      signals.push(state);
  ...
  try {
    ...
    signals.push(fallbackState);
    if (fallbackState.authoritative) return fallbackState;
  } catch (fallbackError) {
    errors.push(fallbackError);
  }

  const resolved = resolveLiveState(signals);
  if (resolved.authoritative || signals.length) return resolved;
  if (errors.length) throw new Error(errors.map(error => error?.message || String(error)).join('; '));
  return emptyState('soop-channel');
}

module.exports = {
  DEFAULT_BROAD_URL,
  DEFAULT_BROAD_FALLBACK_URL,
  DEFAULT_PLAYER_URL,
