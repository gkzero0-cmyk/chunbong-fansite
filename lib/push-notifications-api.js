...
function hasVapid(){const e=vapidEnv();return Boolean(e.publicKey&&e.privateKey);}
...
async function sender(){
  ...
  const webpush=require('web-push'),v=vapidEnv();
...
function authorizedCron(req){
  const secret=String(process.env.CRON_SECRET||'');if(!secret)return false;
  return String(header(req,'authorization')||'')===`Bearer ${secret}`;
}
async function dispatch(){
  if(!hasRedis()||!hasVapid())return{available:false,sent:0};
...
async function handleConfig(req,res){
  ...
  const v=vapidEnv();
  return sendJson(res,200,{available:hasRedis()&&hasVapid(),publicKey:hasVapid()?v.publicKey:''});
}
async function handleSubscription(req,res){
  ...
  try{
    ...
    if(body.action==='subscribe'){
      if(!hasVapid())return sendJson(res,503,{error:'push_not_configured'});
...
module.exports={handleConfig,handleSubscription,handleDispatch,dispatch,_internals:{normalizedPreferences,classifySchedule,validSubscription,subId}};
