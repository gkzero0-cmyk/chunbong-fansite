...
const GITHUB_OIDC_AUDIENCE='chunbong-fansite-push';
const GITHUB_REPOSITORY='gkzero0-cmyk/chunbong-fansite';
const GITHUB_PUSH_WORKFLOW='.github/workflows/push-dispatch.yml@refs/heads/main';
...
async function githubJwks(){
  ...
  const response=await fetch(GITHUB_OIDC_ISSUER+'/.well-known/jwks',{headers:{accept:'application/json'}});
  if(!response.ok)throw new Error('github_oidc_jwks_'+response.status);
  const payload=await response.json(),keys=Array.isArray(payload?.keys)?payload.keys:[];
  githubJwksCache={expiresAt:Date.now()+60*60*1000,keys};
  return keys;
}
async function authorizedGitHubOidc(req){
  const auth=String(header(req,'authorization')||'');
  if(!auth.startsWith('Bearer '))return false;
  const token=auth.slice(7).trim(),parts=token.split('.');
  if(parts.length!==3)return false;
  const [encodedHeader,encodedPayload,encodedSignature]=parts;
  const jwtHeader=decodeJwtPart(encodedHeader),claims=decodeJwtPart(encodedPayload);
  if(!jwtHeader||!claims||jwtHeader.alg!=='RS256'||!jwtHeader.kid)return false;
  const now=Math.floor(Date.now()/1000);
  const audiences=Array.isArray(claims.aud)?claims.aud:[claims.aud];
  if(claims.iss!==GITHUB_OIDC_ISSUER||!audiences.includes(GITHUB_OIDC_AUDIENCE))return false;
  if(Number(claims.exp||0)<=now||Number(claims.nbf||0)>now+30)return false;
  if(String(claims.repository||'')!==GITHUB_REPOSITORY)return false;
  ...
  if(String(claims.workflow_ref||'')!==GITHUB_REPOSITORY+'/'+GITHUB_PUSH_WORKFLOW)return false;
  try{
    const keys=await githubJwks(),jwk=keys.find(key=>key.kid===jwtHeader.kid);
    if(!jwk)return false;
...
async function authorizedDispatch(req){
  const auth=String(header(req,'authorization')||'');
  ...
  if(cronSchedule&&userAgent.includes('vercel'))return true;
  return authorizedGitHubOidc(req);
}
...
async function handleConfig(req,res){
  ...
  return sendJson(res,200,{available:Boolean(hasRedis()&&v?.publicKey),publicKey:v?.publicKey||'',managed:!hasEnvVapid()&&Boolean(v?.publicKey),dispatchAuth:'github-oidc-v1'});
}
...
async function handleDispatch(req,res){
  if(!(await authorizedDispatch(req)))return sendJson(res,401,{error:'unauthorized'});
  if(hasRedis()){
    try{
...
module.exports={handleConfig,handleSubscription,handleDispatch,dispatch,_internals:{normalizedPreferences,classifySchedule,validSubscription,subId,resolveVapid,authorizedGitHubOidc}};
