const { SOOP_ID, BOARD_NUMBER, NOTICE_BOARD_NUMBERS, getJson, listFrom, normalizePost } = require('./_shared');
...
async function fetchHostPage(host, page) {
  ...
  try {
    ...
    return [...listFrom(payload), ...pinned]
      ...
      .filter(item => NOTICE_BOARD_NUMBERS.includes(item.boardNumber));
  } catch (_) {
    return [];
  }
}
