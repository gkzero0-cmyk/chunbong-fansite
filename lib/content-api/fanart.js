...
module.exports = async function fetchFanart() {
  ...
  let raw=[],lastError; for (const url of urls) { try { raw=articles(await getJson(url,naverHeaders,4000)); if(raw.length) break; } catch(e){lastError=e;} }
  if(!raw.length&&lastError) throw lastError;
  // Keep the list request cheap. Naver's board list often omits thumbnails;
  // resolving every missing thumbnail here used to fan out into up to 12
  // article-detail requests and made the whole page wait. The browser now
  // hydrates only visible cards through the separately cached detail route.
  return raw.slice(0,12).map(normalize).filter(item=>item.id||item.title);
};
