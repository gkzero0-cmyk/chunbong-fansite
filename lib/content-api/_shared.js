...
async function getJson(url, requestHeaders = soopHeaders, timeoutMs = 8000) {
...
function normalizePost(item) {
  ...
  const thumb=imageUrl(photos[0]?.url||photos[0]?.src||deepFirst(item,['thumb','thumbnail','image_url','imageUrl']))||'';
  return { id: id ? String(id) : '', boardNumber: boardValue === undefined || boardValue === null || boardValue === '' ? '' : String(boardValue), category: 'NOTICE', title: clean(first(item, ['title','subject','title_name','titleName']) || '춘봉 공지'), date: normalizeDate(rawDate), sortDate: rawDate ? String(rawDate) : '', thumb: thumb.startsWith('//')?'https:'+thumb:thumb, desc: content.slice(0,180), content, link: id ? `https://www.sooplive.com/station/${SOOP_ID}/post/${id}` : `https://www.sooplive.com/station/${SOOP_ID}/board/${BOARD_NUMBER}` };
}
