...
async function fetchChannelBatch(type,{continuation=''}={}){
  if(!['videos','shorts'].includes(type))throw new Error(`unsupported youtube tab ${type}`);
  const html=await getText(`${CHANNEL}/${type}?hl=ko&gl=KR`);
  const config=extractInnertubeConfig(html);
  if(!continuation){
    const data=extractInitialData(html);
    if(!data)return{items:[],nextToken:'',done:true};
    const page=collectBrowsePage(data,type);
    return{items:page.items.filter(item=>item.kind===type),nextToken:page.nextToken||'',done:!page.nextToken};
  }
  const payload=await fetchBrowseContinuation(continuation,config);
  if(!payload)return{items:[],nextToken:'',done:true};
...
module.exports.fetchChannelBatch = fetchChannelBatch;
