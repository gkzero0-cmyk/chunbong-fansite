...
function makeEntry(config) {
  ...
  const sourceId = cleanText(item.id) || fallbackId([item.title, item.sortDate || item.dateIso || item.date, config.index || 0].join('|'));
  const rawWhen = item.dateIso || item.sortDate || item.date || '';
