...
function finalizeSession(state, offlineAt = new Date().toISOString()) {
  ...
  return {
    ...
    broadcastId: String(session.broadcastId || samples.at(-1)?.broadcastId || ''),
    startedAt,
    endedAt,
...
function aggregateCategories(sessions = []) {
  ...
  return [...groups.values()].map(group => ({
    ...
    maxViewers: group.maxViewers,
    sharePercent: totalMinutes > 0 ? round(group.minutes / totalMinutes * 100, 1) : 0
  })).sort((a, b) => b.minutes - a.minutes || a.name.localeCompare(b.name, 'ko'));
}
...
function exactSnapshotValue(snapshots = [], date = '', key = '') {
  const rows = (Array.isArray(snapshots) ? snapshots : [])
    .filter(item => String(item?.date || '').slice(0, 10) === String(date))
    .slice()
...
function latestMonthlySnapshotValue(snapshots = [], month = '', key = '') {
  const rows = (Array.isArray(snapshots) ? snapshots : [])
...
function mergeFollowerSnapshots(snapshots = [], followerHistory = []) {
  const byDate = new Map();
  for (const point of Array.isArray(followerHistory) ? followerHistory : []) {
    const date = String(point?.date || '').slice(0, 10);
    const followerCount = finite(point?.followerCount ?? point?.soop?.followerCount);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || followerCount === null) continue;
    byDate.set(date, { date, capturedAt: point?.capturedAt || `${date}T15:00:00.000Z`, soop: { followerCount } });
  }
  for (const snapshot of Array.isArray(snapshots) ? snapshots : []) {
    const date = String(snapshot?.date || '').slice(0, 10);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) continue;
    const existing = byDate.get(date) || { date, capturedAt: snapshot?.capturedAt || '', soop: {} };
    const snapshotSoop = snapshot?.soop && typeof snapshot.soop === 'object' ? snapshot.soop : {};
    const mergedSoop = {
      ...(existing.soop || {}),
      ...snapshotSoop
    };
    if (finite(snapshotSoop.followerCount) === null && finite(existing?.soop?.followerCount) !== null) {
      mergedSoop.followerCount = existing.soop.followerCount;
    }
    byDate.set(date, {
      ...existing,
      ...snapshot,
...
function aggregateDaily(sessions = [], snapshots = []) {
  ...
  return [...byDate.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([date, rows]) => {
    ...
    return {
      ...
      fanclubCount: exactSnapshotValue(snapshots, date, 'fanclubCount'),
...
function aggregateMonthly(sessions = [], snapshots = []) {
  ...
  return [...groups.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([month, rows]) => {
    ...
    return {
      ...
      fanclubCount: latestMonthlySnapshotValue(snapshots, month, 'fanclubCount'),
...
function recentThreeMonthStart(now = new Date()) {
  const key = kstDateKey(now);
  if (!key) return '';
  const [year, month] = key.slice(0, 7).split('-').map(Number);
  const date = new Date(Date.UTC(year, month - 3, 1));
  return `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, '0')}-01`;
}

function projectLiveSession(live = {}, now = new Date()) {
  if (live?.live !== true || !live?.startedAt) return null;
  ...
  return {
    id: `live-${String(live?.broadcastId || live.startedAt)}`,
    broadcastId: String(live?.broadcastId || ''),
    date: kstDateKey(live.startedAt),
    startedAt: String(live.startedAt),
    endedAt: now instanceof Date ? now.toISOString() : new Date(now).toISOString(),
    durationMinutes,
    averageViewers: null,
...
function sessionsInDateRange(sessions = [], startDate = '', endDate = '') {
...
function buildSoopAnalytics(sessions = [], snapshots = [], live = {}, now = new Date(), options = {}) {
  ...
  const combinedSnapshots = mergeFollowerSnapshots(snapshots, options?.followerHistory || []);
  const projection = projectLiveSession(live, now);
  const projectionAlreadyFinalized = projection && projection.broadcastId
    ? cleanSessions.some(item => String(item?.broadcastId || '') === projection.broadcastId)
    : false;
  const displaySessions = projection && !projectionAlreadyFinalized ? [...cleanSessions, projection] : cleanSessions.slice();
  const daily = aggregateDaily(displaySessions, combinedSnapshots);
  const monthly = aggregateMonthly(displaySessions, combinedSnapshots);
  ...
  const recentStart = recentThreeMonthStart(now);
  const recentCategories = aggregateCategories(sessionsInDateRange(displaySessions, recentStart, todayKey));
  return {
    overview: {
      live: live?.live ?? null,
      broadcastId: String(live?.broadcastId || ''),
      ...
      followerCount: liveFollower !== null ? liveFollower : latestSnapshotValue(combinedSnapshots, 'followerCount'),
      fanclubCount: liveFanclub !== null ? liveFanclub : latestSnapshotValue(combinedSnapshots, 'fanclubCount'),
    ...
    categories: aggregateCategories(displaySessions),
    categoryPeriods: {
      recentThreeMonths: recentCategories,
      recentThreeMonthsStart: recentStart,
