...
const ARCHIVE_TYPES=new Set(['series','edition']);
...
function normalizeArchiveItem(raw={}){
  ...
  return {
    ...
    archiveType:ARCHIVE_TYPES.has(raw.archiveType)?raw.archiveType:'series',parentId:text(raw.parentId,80).toLowerCase().replace(/[^a-z0-9가-힣-]/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,''),editionLabel:text(raw.editionLabel,80),editionOrder:Number.isFinite(Number(raw.editionOrder))?Number(raw.editionOrder):0,featured:raw.featured===true,
...
function validateArchiveItem(item,{publishing=false}={}){
  ...
  if(!ARCHIVE_TYPES.has(item.archiveType))errors.push('invalid_archive_type');
  if(item.archiveType==='edition'&&!item.parentId)errors.push('edition_parent_required');
  if(item.archiveType==='series'&&item.parentId)errors.push('series_parent_not_allowed');
...
function validateArchiveRelationships(rows=[]){
  const normalized=(Array.isArray(rows)?rows:[]).map(normalizeArchiveItem);
  const ids=new Set(normalized.map(row=>row.id).filter(Boolean));
  const errors=[];
  for(const row of normalized){
    if(row.archiveType==='edition'&&(!row.parentId||!ids.has(row.parentId)))errors.push('unknown_parent_id');
    if(row.archiveType==='series'&&row.parentId)errors.push('series_parent_not_allowed');
    if(row.parentId===row.id&&row.id)errors.push('self_parent_id');
  }
  return [...new Set(errors)];
}
...
module.exports={normalizeArchiveItem,validateArchiveItem,validateArchiveRelationships,toPublicArchiveItem,formatArchiveDate,TYPES,PRECISIONS,VERIFY_STATES,ARCHIVE_TYPES};
