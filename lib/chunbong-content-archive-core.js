...
function toPublicArchiveItem(raw){
  ...
  const cleanMaterial=row=>internalIds.has(row.sourceId)?{...row,sourceId:'',url:''}:row;
  const visibleMaterial=row=>!(row.type==='reference'&&internalIds.has(row.sourceId));
  const timeline=item.timeline.filter(visibleMaterial).map(cleanMaterial);
  const media=item.media.filter(visibleMaterial).map(cleanMaterial);
  ...
  const participantGroups=item.participantGroups.map(({sourceId,...row})=>row);
  return {...publicItem,heroImage,timeline,media,gallery,seriesSessions,participantGroups,sources:visibleSources,verifiedAt:verification.verifiedAt,sourceCount:visibleSources.length};
}
