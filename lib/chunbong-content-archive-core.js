...
function normalizeSource(row={}){
  return {id:text(row.id,80),kind:text(row.kind,40)||'reference',label:text(row.label,120),url:normalizeUrl(row.url),visibility:row.visibility==='internal'?'internal':'public'};
}
...
function normalizeParticipantGroup(row={}){
  const participants=list(row.participants).map(v=>text(v,100)).filter(Boolean);
...
function normalizeArchiveItem(raw={}){
  ...
  return {
    ...
    participants:list(raw.participants).map(v=>text(v,80)).filter(Boolean),participantGroups:list(raw.participantGroups).map(normalizeParticipantGroup).filter(row=>row.id||row.title),results:list(raw.results),
...
function toPublicArchiveItem(raw){
  ...
  const sources=item.sources.filter(row=>row.visibility!=='internal');
  const timeline=item.timeline.filter(row=>row.visibility!=='internal');
  const media=item.media.filter(row=>row.visibility!=='internal');
  return {...publicItem,sources,timeline,media,verifiedAt:verification.verifiedAt,sourceCount:sources.length};
}
module.exports={normalizeArchiveItem,normalizeParticipantGroup,normalizeSeriesInfo,normalizeSeriesSession,validateArchiveItem,toPublicArchiveItem,formatArchiveDate,TYPES,PRECISIONS,VERIFY_STATES};
