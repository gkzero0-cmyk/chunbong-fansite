...
const STATUSES=new Set(['upcoming','recruiting','ongoing','ended']);
...
function normalizeArchiveItem(raw={}){
  ...
  return {
    ...
    participants:list(raw.participants).map(v=>text(v,80)).filter(Boolean),participantCount:Math.max(0,Math.floor(Number(raw.participantCount)||0)),results:list(raw.results),
