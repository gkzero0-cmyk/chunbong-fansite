...
function normalizeSourceUrl(value=''){
  ...
  try{
    ...
    if(host==='bngts.com'||host==='www.bngts.com')return'';
...
function normalizeSource(row={}){
  ...
  if(!url&&/bngts\.com/i.test(String(row.url||'')))return null;
...
function normalizeArchiveItem(raw={}){
  ...
  return {
    ...
    notionSections:list(raw.notionSections).map(normalizeNotionSection).filter(row=>row.id||row.title||row.text),
...
function publicSourceSafeText(value=''){
  return String(value||'').split(/(?<=[.!?。])\s+/).filter(part=>!/방통실|BNGTS/i.test(part)).join(' ').trim();
}
function blockedArchiveSourceUrl(value=''){return /bngts\.com|namu\.moe/i.test(String(value||''))}
function toPublicArchiveItem(raw){
  const item=normalizeArchiveItem(raw);const {verification,...publicItem}=item;
  const sources=item.sources.filter(row=>row.visibility!=='internal'&&!/bngts\.com|namu\.moe/i.test(String(row.url||'')));
  const timeline=item.timeline.filter(row=>row.visibility!=='internal'&&!/bngts\.com|namu\.moe/i.test(String(row.url||'')));
  const media=item.media.filter(row=>row.visibility!=='internal'&&!/bngts\.com|namu\.moe/i.test(String(row.url||'')));
  const results=(item.results||[]).filter(row=>!/방통실|BNGTS/i.test(JSON.stringify(row||{})));
  const participantGroups=(item.participantGroups||[]).filter(row=>!/bngts/i.test(String(row.sourceId||''))&&!/방통실|BNGTS/i.test(String(row.note||'')));
  const participants=item.id==='justserver-moneygame'?[]:item.participants;
  return {...publicItem,description:publicSourceSafeText(item.description),results,participantGroups,participants,sources,timeline,media,verifiedAt:verification.verifiedAt,sourceCount:sources.length};
}
module.exports={normalizeArchiveItem,normalizeSeriesInfo,normalizeSeriesSession,normalizeParticipantGroup,validateArchiveItem,toPublicArchiveItem,blockedArchiveSourceUrl,formatArchiveDate,TYPES,PRECISIONS,VERIFY_STATES};
