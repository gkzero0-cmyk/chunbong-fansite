...
function normalizeNotionImage(row={}){
  ...
  return {
    ...
    originId:text(row.originId||row.blockId,160),
    provider:text(row.provider,40),
    assetState:row.assetState==='permanent'?'permanent':'remote',
    assetHash:text(row.assetHash,128),
    width:Number.isFinite(width)&&width>0?Math.floor(width):0,
    height:Number.isFinite(height)&&height>0?Math.floor(height):0
  };
}
function normalizeGuideContentBlock(row={}){
  if(!row||typeof row!=='object')return null;
  if(row.type==='image'){
    const image=normalizeNotionImage(row.image||row);
    return image?{type:'image',image}:null;
  }
  const value=text(row.text,3000);
  return value?{type:'text',text:value}:null;
}
function normalizeDocumentSection(row={},provider='notion'){
  const depth=Number(row.depth);
  return {
    id:text(row.id,120),
    sourceId:text(row.sourceId,80),
    pageId:text(row.pageId,120),
    ...
    content:list(row.content).map(normalizeGuideContentBlock).filter(Boolean).slice(0,160),
    provider:text(row.provider||provider,40),
    depth:Number.isFinite(depth)&&depth>=0?Math.min(12,Math.floor(depth)):0
  };
}
function normalizeNotionSection(row={}){return normalizeDocumentSection(row,'notion')}
function normalizeReferenceSection(row={}){return normalizeDocumentSection(row,row.provider||'reference')}
