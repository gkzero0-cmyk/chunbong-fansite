...
function normalizeParticipantProfile(row={}){
  if(!row||typeof row!=='object')return null;
  const canonicalName=text(row.canonicalName||row.name||row.displayName,80);
  if(!canonicalName)return null;
  const displayName=text(row.displayName||canonicalName,80)||canonicalName;
  const aliases=[...new Set(list(row.aliases).map(v=>text(v,80)).filter(Boolean).filter(v=>v!==canonicalName&&v!==displayName))].slice(0,24);
  const role=['participant','npc','staff','host','unknown'].includes(row.role)?row.role:'participant';
  return {
    canonicalName,displayName,aliases,
    rpName:text(row.rpName||row.roleplayName,100),
    platform:text(row.platform,80),
    entryRound:text(row.entryRound||row.stage,120),
    role,
    sourceId:text(row.sourceId,80)
  };
}
function mergeParticipantProfiles(groups=[],rows=[],direct=[]){
  const out=[],byName=new Map();
  const key=value=>String(value||'').normalize('NFKC').toLocaleLowerCase('ko-KR').trim();
  const add=raw=>{
    const profile=normalizeParticipantProfile(raw);if(!profile)return;
    const k=key(profile.canonicalName),index=byName.get(k);
    if(index==null){byName.set(k,out.length);out.push(profile);return}
    const current=out[index];
    out[index]={
      ...current,
      ...Object.fromEntries(Object.entries(profile).filter(([name,value])=>name==='aliases'||value)),
      canonicalName:current.canonicalName,
      displayName:profile.displayName||current.displayName,
      aliases:[...new Set([...(current.aliases||[]),...(profile.aliases||[])])].filter(v=>v!==current.canonicalName).slice(0,24)
    };
  };
  for(const group of groups||[])for(const name of group.participants||[])add({canonicalName:name,displayName:name,platform:group.platform,entryRound:group.stage,sourceId:group.sourceId,role:'participant'});
  for(const name of direct||[])add({canonicalName:name,displayName:name,role:'participant'});
...
function normalizeNotionImage(row={}){
  ...
  return {
    ...
    assetState:['permanent','remote','proxy'].includes(row.assetState)?row.assetState:'',
...
function normalizeArchiveItem(raw={}){
  ...
  const participants=list(raw.participants).map(v=>text(v,80)).filter(Boolean);
  const participantGroups=list(raw.participantGroups).map(normalizeParticipantGroup).filter(row=>row.id||row.title||row.participants.length);
  const participantProfiles=mergeParticipantProfiles(participantGroups,list(raw.participantProfiles),participants);
  return {
    ...
    participants,participantGroups,participantProfiles,results:list(raw.results),
...
function validateArchiveItem(item,{publishing=false}={}){
  ...
  const profileNames=item.participantProfiles.map(row=>String(row?.canonicalName||'').toLocaleLowerCase('ko-KR').trim()).filter(Boolean);
  if(new Set(profileNames).size!==profileNames.length)errors.push('duplicate_participant_profile');
  ...
  const referencedSourceIds=[
    ...
    ...item.participantProfiles.map(row=>row?.sourceId),
...
function publicSourceSafeText(value=''){
  return String(value||'')
    ...
    .replace(/BNGTS/gi,'')
    .replace(/Nemopix/gi,'')
    .replace(/FM\s*코리아|FM\s*Korea|fmkorea(?:\.com)?/gi,'')
    .replace(/[ \t]{2,}/g,' ')
    .replace(/\s+([,.;:!?])/g,'$1')
    .trim();
}
...
function humanizeGuideImageLabel(value=''){
  ...
  if(imageFilenameOnly(raw)){
    raw=raw.split(/[\\/]/).pop().replace(/\?.*$/,'').replace(/\.(?:png|jpe?g|webp|gif|svg|avif)$/i,'').trim();
    if(/^[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}$/i.test(raw))raw='';
    else raw=raw.replace(/[_-]+/g,' ').replace(/\s+/g,' ').trim();
  }
...
function cleanGuideImage(image={},hiddenSourceIds=new Set()){
  const src=String(image.src||''),provider=String(image.provider||'').toLowerCase();
  if(provider==='namuwiki'&&image.permanent!==true&&/https?:\/\/(?:[^/]+\.)?namu\.moe\//i.test(src))return null;
...
function blockedArchiveSourceUrl(value=''){return /bngts\.com|namu\.moe|nemopix\.xyz|fmkorea\.com/i.test(String(value||''))}
function toPublicArchiveItem(raw){
  ...
  const participantProfiles=(item.participantProfiles||[]).map(row=>hiddenSourceIds.has(String(row.sourceId||''))?{...row,sourceId:''}:row);
  ...
  return {...publicItem,description:publicSourceSafeText(item.description),results,participantGroups,participantProfiles,participants,sources,timeline,media,notionSections,referenceSections,knowledgeSections,verifiedAt:verification.verifiedAt,sourceCount:sources.length};
}
module.exports={normalizeArchiveItem,normalizeSeriesInfo,normalizeSeriesSession,normalizeParticipantGroup,normalizeParticipantProfile,mergeParticipantProfiles,validateArchiveItem,toPublicArchiveItem,blockedArchiveSourceUrl,formatArchiveDate,cleanGuideText,imageFilenameOnly,humanizeGuideImageLabel,TYPES,PRECISIONS,VERIFY_STATES};
