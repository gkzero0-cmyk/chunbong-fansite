...
function toPublicArchiveItem(raw){
  ...
  const referenceSections=(item.referenceSections||[]).map(section=>({
    ...section,
    sourceId:hiddenSourceIds.has(String(section.sourceId||''))?'':section.sourceId,
    text:publicSourceSafeText(section.text),
    images:(section.images||[]).map(image=>hiddenSourceIds.has(String(section.sourceId||''))?{...image,sourceId:''}:image),
    content:(section.content||[]).map(block=>block?.type==='text'?{...block,text:publicSourceSafeText(block.text)}:block)
  }));
  ...
  return {...publicItem,description:publicSourceSafeText(item.description),results,participantGroups,participants,sources,timeline,media,referenceSections,knowledgeSections,verifiedAt:verification.verifiedAt,sourceCount:sources.length};
}
