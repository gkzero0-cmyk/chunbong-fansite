...
function stripGuideImageFilenameTokens(value=''){
  return String(value||'')
    .replace(/(?:^|[\s([{])(?:[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}|[0-9]{4}-[0-9]{2}-[0-9]{2}[_ .-][0-9.:-]{4,}|[A-Za-z0-9가-힣][A-Za-z0-9가-힣 _().-]{0,150})\.(?:png|jpe?g|webp|gif|svg|avif)(?=$|[\s)\]},.;:])/gi,' ')
    .replace(/[ \t]{2,}/g,' ');
}
function cleanNamuWikiText(value=''){
  return stripGuideImageFilenameTokens(String(value||'')
    .replace(/#!if\b[^\n]{0,500}?(?=(?:\[\[파일:|<col|<row|<table|상위 문서|관련 문서|$))/gi,' ')
    .replace(/\[\[파일:[^\]]*\]\]/gi,' ')
...
function cleanGuideText(value='',provider=''){
  const source=provider==='namuwiki'?cleanNamuWikiText(value):stripGuideImageFilenameTokens(value);
  const lines=source.split(/\r?\n/).map(line=>line.trim()).filter(line=>line&&!imageFilenameOnly(line)).map(line=>{
    let clean=line;
    if(provider==='namuwiki')clean=clean
      .replace(/(?:#!(?:if|wiki|folding|html)\b|<\/?(?:nopad|row|col|table|width|tablewidth|tablebgcolor|tablebordercolor)[^>]*>|\{\{\{|\}\}\}|\|\|)/gi,' ')
      .replace(/(?:^|\s)<(?:col|row|table|width)[^>]*>/gi,' ');
    clean=stripGuideImageFilenameTokens(clean).replace(/[ \t]{2,}/g,' ').trim();
    return clean?publicSourceSafeText(clean):'';
  }).filter(Boolean);
  return lines.join('\n').replace(/\n{3,}/g,'\n\n').trim();
}
...
module.exports={normalizeArchiveItem,normalizeSeriesInfo,normalizeSeriesSession,normalizeParticipantGroup,validateArchiveItem,toPublicArchiveItem,blockedArchiveSourceUrl,formatArchiveDate,cleanGuideText,stripGuideImageFilenameTokens,imageFilenameOnly,humanizeGuideImageLabel,TYPES,PRECISIONS,VERIFY_STATES};
