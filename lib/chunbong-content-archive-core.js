...
function normalizeMaterial(row={}){
  ...
  return {
    ...
    date:text(row.date,10),datePrecision:precision,url:normalizeUrl(row.url),thumbnail:normalizeUrl(row.thumbnail),
    sourceId:text(row.sourceId,80),note:text(row.note,1000)
  };
}
function normalizeGallery(row={}){
  return {id:text(row.id,80),src:normalizeUrl(row.src||row.url),url:normalizeUrl(row.url),alt:text(row.alt,200),caption:text(row.caption,300),sourceId:text(row.sourceId,80)};
}
function normalizeArchiveItem(raw={}){
  ...
  return {
    ...
    heroImage:raw.heroImage&&typeof raw.heroImage==='object'?{src:normalizeUrl(raw.heroImage.src),alt:text(raw.heroImage.alt,200),sourceId:text(raw.heroImage.sourceId,80)}:null,
...
function validDateForPrecision(value,precision){
  if(precision==='unknown')return !value||/^\d{4}(?:-\d{2}(?:-\d{2})?)?$/.test(value);
...
function validateArchiveItem(item,{publishing=false}={}){
  ...
  if(!validDateForPrecision(item.startDate,item.datePrecision))errors.push('invalid_start_date');
  const urls=[...item.timeline,...item.media].map(row=>String(row?.url||'').trim()).filter(Boolean);
