...
function validEventType(value){return['page_view','active_time','menu_click','navigation_timing','game_start','game_finish','tarot_start','tarot_result','schedule_open','data_tab_open','feedback_open','feedback_submit'].includes(String(value||''))}
...
async function analyticsOverview(days=7){
  ...
  const pages={},pageTime={},menus={},devices={},features={},hours={},perfTotal={},perfCount={};
  let pageviews=0,activeMs=0,newVisitors=0,returningVisits=0,visitors=0,sessions=0;
  if(all){
    const keys=analyticsKeys('all');
    const [summaryRaw,pageRaw,pageTimeRaw,menuRaw,deviceRaw,featureRaw,hourRaw,perfTotalRaw,perfCountRaw,v,s]=await Promise.all([
    ...
    sumHash(pages,pageRaw);sumHash(pageTime,pageTimeRaw);sumHash(menus,menuRaw);sumHash(devices,deviceRaw);sumHash(features,featureRaw);sumHash(hours,hourRaw);sumHash(perfTotal,perfTotalRaw);sumHash(perfCount,perfCountRaw);
  }else{
    for(const day of aggregateDates){
      const keys=analyticsKeys(day);
      const [summaryRaw,pageRaw,pageTimeRaw,menuRaw,deviceRaw,featureRaw,hourRaw,perfTotalRaw,perfCountRaw]=await Promise.all([
        redisCommand('HGETALL',keys.summary),redisCommand('HGETALL',keys.pages),redisCommand('HGETALL',keys.pageTime),redisCommand('HGETALL',keys.menus),
        redisCommand('HGETALL',keys.devices),redisCommand('HGETALL',keys.features),redisCommand('HGETALL',keys.hours),redisCommand('HGETALL',keys.perfTotal),redisCommand('HGETALL',keys.perfCount)
      ]);
      const summary=hashObject(summaryRaw);
  ...
  const performancePages=Object.keys(perfCount).map(key=>({key,samples:Number(perfCount[key])||0,totalMs:Number(perfTotal[key])||0})).filter(row=>row.samples>0).map(row=>({...row,averageMs:Math.round(row.totalMs/row.samples)})).sort((a,b)=>b.averageMs-a.averageMs||b.samples-a.samples).slice(0,20);
  const performanceSamples=performancePages.reduce((sum,row)=>sum+row.samples,0),performanceTotalMs=performancePages.reduce((sum,row)=>sum+row.totalMs,0);
  return{
    ...
    funnel,comparison,
    performance:{averageMs:performanceSamples?Math.round(performanceTotalMs/performanceSamples):0,samples:performanceSamples,pages:performancePages},
...
async function githubProjectState(){
  ...
  try{
    ...
    return{available:Boolean(repo||commit),mainSha,repoSizeKb:Number(repo?.size)||0,updatedAt:repo?.updated_at||null,vercelStatus};
  }catch{return{available:false,mainSha:'',repoSizeKb:0,updatedAt:null,vercelStatus:null}}finally{clearTimeout(timer)}
}
...
async function operatorSystemStatus(req){
  ...
  endpoints.filter(row=>row.ok&&Number(row.ms)>=1500).forEach(row=>healthIssues.push((row.label||'API')+' 응답 지연 '+row.ms+'ms'));
