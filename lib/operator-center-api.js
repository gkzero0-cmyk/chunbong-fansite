...
async function session(req){return verifyToken(cookies(req)[SESSION_COOKIE],'session')}
async function setSession(res,provider,extra={}){
  const token=await signToken({purpose:'session',owner:true,provider,githubId:OWNER_GITHUB_ID,iat:Date.now(),exp:Date.now()+SESSION_SECONDS*1000,...extra});
  setHeader(res,'Set-Cookie',`${SESSION_COOKIE}=${encodeURIComponent(token)}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=7776000`);
}
...
function ownerEmail(value){return sha256(String(value||'').trim().toLowerCase())===OWNER_EMAIL_SHA256}
...
function analyticsKeys(day){
  ...
  return{
    ...
    devices:`${p}devices:v1:${day}`,features:`${p}features:v1:${day}`,hours:`${p}hours:v1:${day}`,
...
function validEventType(value){return['page_view','active_time','menu_click','game_start','game_finish','tarot_start','tarot_result','schedule_open','data_tab_open','feedback_open'].includes(String(value||''))}
async function ingestAnalytics(body){
  ...
  const now=new Date(),day=dayKst(now),hour=hourKst(now),keys=analyticsKeys(day);
  const visitorHash=sha256('visitor:'+visitorId).slice(0,32),sessionHash=sha256('session:'+sessionId).slice(0,32);
  const commands=[
    ['SET',ANALYTICS_START_KEY,new Date().toISOString(),'NX'],
    ['PFADD',keys.visitors,visitorHash],['PFADD',keys.sessions,sessionHash],
  ...
  for(const event of events){
    ...
    if(type==='page_view'){
      commands.push(['HINCRBY',keys.summary,'pageviews',1],['HINCRBY',keys.pages,page,1],['HINCRBY',keys.hours,hour,1]);
      const device=['mobile','tablet','desktop'].includes(event.device)?event.device:'other';
      commands.push(['HINCRBY',keys.devices,'device:'+device,1]);
      commands.push(['HINCRBY',keys.devices,'pwa:'+(event.pwa?'yes':'no'),1]);
      commands.push(['HINCRBY',keys.devices,'theme:'+(event.theme==='light'?'light':'dark'),1]);
      if(event.visitorState==='new')commands.push(['HINCRBY',keys.summary,'newVisitors',1]);
      else if(event.visitorState==='returning')commands.push(['HINCRBY',keys.summary,'returningVisits',1]);
    }else if(type==='active_time'){
      const ms=Math.max(0,Math.min(60000,Math.round(Number(event.activeMs)||0)));
      if(ms)commands.push(['HINCRBY',keys.summary,'activeMs',ms]);
    }else if(type==='menu_click'){
      const target=safeText(event.target,80)||'unknown';commands.push(['HINCRBY',keys.menus,target,1]);
    }else{
      const target=safeText(event.target,80);
      commands.push(['HINCRBY',keys.features,target?`${type}:${target}`:type,1]);
    }
  }
...
function transientRequestHash(req){
  ...
  return sha256('feedback-rate:'+raw).slice(0,28);
}
async function feedbackRateLimit(req,visitorId=''){
  if(!hasRedis())return false;
  const parts=[transientRequestHash(req)];
...
async function saveFeedback(req,body){
  ...
  const row={
    id,category,nickname,message,status:'new',createdAt:now.toISOString(),updatedAt:now.toISOString(),
...
async function updateFeedback(id,status){
  if(!FEEDBACK_STATUS.has(status))throw new Error('invalid_status');
  ...
  await redisPipeline([
    ['SET',FEEDBACK_PREFIX+row.id,JSON.stringify(row)],
    ['HINCRBY',FEEDBACK_COUNTS,previous,-1],
    ['HINCRBY',FEEDBACK_COUNTS,status,1]
  ]);
  return row;
}
async function analyticsOverview(days=7){
  const count=[1,7,30].includes(Number(days))?Number(days):7,dates=recentDays(count);
  const daily=[],pages={},menus={},devices={},features={},hours={};
  let pageviews=0,activeMs=0,newVisitors=0,returningVisits=0;
  for(const day of dates){
    const keys=analyticsKeys(day);
    const [summaryRaw,pageRaw,menuRaw,deviceRaw,featureRaw,hourRaw]=await Promise.all([
      redisCommand('HGETALL',keys.summary),redisCommand('HGETALL',keys.pages),redisCommand('HGETALL',keys.menus),
      redisCommand('HGETALL',keys.devices),redisCommand('HGETALL',keys.features),redisCommand('HGETALL',keys.hours)
    ]);
    const summary=hashObject(summaryRaw),pv=Number(summary.pageviews)||0,am=Number(summary.activeMs)||0;
    pageviews+=pv;activeMs+=am;newVisitors+=Number(summary.newVisitors)||0;returningVisits+=Number(summary.returningVisits)||0;
    sumHash(pages,pageRaw);sumHash(menus,menuRaw);sumHash(devices,deviceRaw);sumHash(features,featureRaw);sumHash(hours,hourRaw);
    const [visitors,sessions]=await Promise.all([redisCommand('PFCOUNT',keys.visitors),redisCommand('PFCOUNT',keys.sessions)]);
    daily.push({date:day,pageviews:pv,visitors:Number(visitors)||0,sessions:Number(sessions)||0,activeMs:am});
  }
  const visitorKeys=dates.map(day=>analyticsKeys(day).visitors),sessionKeys=dates.map(day=>analyticsKeys(day).sessions);
  const [visitors,sessions,start,feedbackCounts]=await Promise.all([
    visitorKeys.length?redisCommand('PFCOUNT',...visitorKeys):0,
    sessionKeys.length?redisCommand('PFCOUNT',...sessionKeys):0,
    redisCommand('GET',ANALYTICS_START_KEY),
    redisCommand('HGETALL',FEEDBACK_COUNTS)
  ]);
  ...
  return{
    periodDays:count,collectionStartedAt:start||null,activeNow,pageviews,
    visitors:Number(visitors)||0,sessions:sessionCount,
    averageActiveSeconds:sessionCount?Math.round(activeMs/sessionCount/1000):0,
    newVisitors,returningVisits,daily,
    topPages:topRows(pages,10),topMenus:topRows(menus,10),devices:topRows(devices,20),
    topFeatures:topRows(features,14),hourly:topRows(hours,24).sort((a,b)=>a.key.localeCompare(b.key)),
...
async function handleSession(req,res){
  ...
  return sendJson(res,200,{authenticated:true,provider:current.provider,expiresAt:new Date(current.exp).toISOString(),owner:{githubLogin:OWNER_GITHUB_LOGIN,githubId:OWNER_GITHUB_ID,emailRegistered:true}});
}
async function handleGithubStart(req,res){
  ...
  const callback=origin(req)+'/api/content?type=operator-github-callback';
...
async function handleGithubCallback(req,res){
  const q=query(req),state=await verifyToken(q.get('state'),'github-state'),code=safeText(q.get('code'),240);
...
async function handleEmailStart(req,res){
  ...
  if(!ownerEmail(email)){
    ...
    return sendJson(res,200,{ok:true,eligible:false});
  }
  return sendJson(res,200,{ok:true,eligible:true,firebase:publicFirebase()});
}
async function handleEmailComplete(req,res){
  ...
  try{
    ...
    if(!response.ok||!user||user.emailVerified===false||!ownerEmail(user.email))throw new Error('owner_mismatch');
...
async function handleOperatorAnalytics(req,res){
  ...
  try{return sendJson(res,200,await analyticsOverview(Number(query(req).get('days')||7)))}catch(error){return sendJson(res,503,{error:'analytics_unavailable'})}
}
...
async function handleOperatorFeedbackUpdate(req,res){
  ...
  if(!sameOrigin(req))return sendJson(res,403,{error:'origin_not_allowed'});
  const body=parseBody(req?.body);if(!body)return sendJson(res,400,{error:'invalid_request'});
  try{return sendJson(res,200,{ok:true,item:await updateFeedback(safeText(body.id,40),String(body.status||''))})}
  catch(error){
    const name=String(error?.message||'feedback_unavailable');
    return sendJson(res,name==='feedback_not_found'?404:name==='invalid_status'?400:503,{error:name});
  }
}
...
module.exports={
  ...
  handleEmailStart,handleEmailComplete,handleOperatorAnalytics,handleOperatorFeedback,handleOperatorFeedbackUpdate,handleLogout,
  _internals:{OWNER_GITHUB_ID,OWNER_GITHUB_LOGIN,OWNER_EMAIL_SHA256,ownerEmail,ingestAnalytics,analyticsOverview,saveFeedback,verifyToken,signToken}
};
