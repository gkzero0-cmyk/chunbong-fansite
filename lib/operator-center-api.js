...
async function setSession(res,provider,extra={}){
  ...
  if(hasRedis()){
    try{await redisPipeline([['ZADD',SESSION_INDEX,exp,jti],['ZREMRANGEBYSCORE',SESSION_INDEX,0,now]])}catch(_){}
  }
...
async function handleSession(req,res){
  ...
  return sendJson(res,200,{authenticated:true,provider:current.provider,expiresAt:new Date(current.exp).toISOString(),activeSessions,owner:{githubLogin:OWNER_GITHUB_LOGIN,githubId:OWNER_GITHUB_ID,emailRegistered:true}});
}
...
async function handleLogout(req,res){
  ...
  if(current?.jti&&hasRedis()){try{await redisCommand('ZREM',SESSION_INDEX,current.jti)}catch(_){}}
...
module.exports={
  ...
  handleEmailStart,handleEmailComplete,handleOperatorAnalytics,handleOperatorFeedback,handleOperatorFeedbackUpdate,handleLogout,handleLogoutAll,
