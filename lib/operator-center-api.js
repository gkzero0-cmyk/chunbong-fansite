...
async function session(req){
  ...
  if(hasRedis()&&current.jti){
    try{
      const score=Number(await redisCommand('ZSCORE',SESSION_INDEX,current.jti));
      if(!Number.isFinite(score)||score<Date.now())return null;
    }catch{return null}
  }
  return current;
}
async function setSession(res,provider,extra={}){
  ...
  if(hasRedis()){
    try{
      const meta={id:jti,provider:safeText(provider,24),createdAt:new Date(now).toISOString(),expiresAt:new Date(exp).toISOString()};
      await redisPipeline([
...
function validEventType(value){return['page_view','active_time','menu_click','game_start','game_finish','tarot_start','tarot_result','schedule_open','data_tab_open','feedback_open','feedback_submit'].includes(String(value||''))}
...
async function updateFeedback(id,status,memo){
  const row=await feedbackItem(id);if(!row)throw new Error('feedback_not_found');
  const nextStatus=status===undefined||status===null||status===''?row.status:String(status);
  ...
  if(memo!==undefined)row.operatorMemo=safeMultiline(memo,2000);
...
async function analyticsOverview(days=7){
  ...
  const chartDates=all?recordedDates.slice(-60):recordedDates;
  const aggregateDates=all?['all']:recordedDates;
  const daily=[];
  ...
  const pages={},pageTime={},menus={},devices={},features={},hours={};
  let pageviews=0,activeMs=0,newVisitors=0,returningVisits=0,visitors=0,sessions=0;
  if(all){
    const keys=analyticsKeys('all');
    const [summaryRaw,pageRaw,pageTimeRaw,menuRaw,deviceRaw,featureRaw,hourRaw,v,s]=await Promise.all([
    ...
    sumHash(pages,pageRaw);sumHash(pageTime,pageTimeRaw);sumHash(menus,menuRaw);sumHash(devices,deviceRaw);sumHash(features,featureRaw);sumHash(hours,hourRaw);
  }else{
    for(const day of aggregateDates){
      const keys=analyticsKeys(day);
      const [summaryRaw,pageRaw,pageTimeRaw,menuRaw,deviceRaw,featureRaw,hourRaw]=await Promise.all([
        redisCommand('HGETALL',keys.summary),redisCommand('HGETALL',keys.pages),redisCommand('HGETALL',keys.pageTime),redisCommand('HGETALL',keys.menus),
        redisCommand('HGETALL',keys.devices),redisCommand('HGETALL',keys.features),redisCommand('HGETALL',keys.hours)
      ]);
      const summary=hashObject(summaryRaw);
  ...
  if(!all){
    ...
    const previous=await analyticsTotalsForDates(compareDates);
  ...
  return{
    ...
    topPages:topRows(pages,10).map(row=>({...row,averageActiveSeconds:row.value?Math.round((Number(pageTime[row.key])||0)/row.value/1000):0})),
    topMenus:topRows(menus,10),devices:topRows(devices,20),
...
async function operatorSessionList(currentJti=''){
  ...
  return Promise.all(pairs.map(async([id,score])=>{
    ...
    return{id,current:id===currentJti,provider:meta.provider||'unknown',createdAt:meta.createdAt||null,expiresAt:meta.expiresAt||new Date(score).toISOString()};
  }));
}
async function githubProjectState(){
  ...
  try{
    ...
    const repo=repoResponse.ok?await repoResponse.json():null,commit=commitResponse.ok?await commitResponse.json():null;
    return{available:Boolean(repo||commit),mainSha:String(commit?.sha||''),repoSizeKb:Number(repo?.size)||0,updatedAt:repo?.updated_at||null};
  }catch{return{available:false,mainSha:'',repoSizeKb:0,updatedAt:null}}finally{clearTimeout(timer)}
}
async function operatorSystemStatus(){
  ...
  const [github,today]=await Promise.all([githubProjectState(),hasRedis()?analyticsOverview(1).catch(()=>null):null]);
  const synced=Boolean(github.mainSha&&deploymentSha&&github.mainSha===deploymentSha);
  return{
    checkedAt:now,
    deployment:{environment:String(process.env.VERCEL_ENV||process.env.NODE_ENV||'unknown'),sha:deploymentSha,url:deploymentUrl,mainSha:github.mainSha,synced},
    storage:{redisConfigured:hasRedis(),redisOk,analyticsRecordedDays:recordedDays,feedbackTotal},
    ...
    repository:{sizeKb:github.repoSizeKb,updatedAt:github.updatedAt},
...
async function handleOperatorFeedbackUpdate(req,res){
  ...
  if(!sameOrigin(req))return sendJson(res,403,{error:'origin_not_allowed'});
  const body=parseBody(req?.body);if(!body)return sendJson(res,400,{error:'invalid_request'});
  try{return sendJson(res,200,{ok:true,item:await updateFeedback(safeText(body.id,40),body.status,body.memo)})}
  catch(error){
    const name=String(error?.message||'feedback_unavailable');
...
async function handleOperatorSystemStatus(req,res){
  const current=await requireOwner(req,res);if(!current)return;
  if(String(req?.method||'GET').toUpperCase()!=='GET')return sendJson(res,405,{error:'method_not_allowed'});
  try{return sendJson(res,200,await operatorSystemStatus())}catch{return sendJson(res,503,{error:'system_status_unavailable'})}
}
...
module.exports={
  ...
  handleEmailStart,handleEmailComplete,handleOperatorAnalytics,handleOperatorFeedback,handleOperatorFeedbackUpdate,handleOperatorSystemStatus,handleOperatorSessionRevoke,handleLogout,handleLogoutAll,
