...
async function ingestAnalytics(body){
  ...
  for(const event of events){
    ...
    if(type==='page_view'){
      ...
      if(ms)commands.push(['HINCRBY',keys.summary,'activeMs',ms],['HINCRBY',allKeys.summary,'activeMs',ms]);
...
async function analyticsOverview(days=7){
  ...
  const chartDates=all
    ? ((await redisCommand('ZRANGE',ANALYTICS_DAYS_KEY,0,-1))||[]).slice(-60)
    : recentDays(count);
  const aggregateDates=all?['all']:chartDates;
  const daily=[];
  ...
  const pages={},menus={},devices={},features={},hours={};
  let pageviews=0,activeMs=0,newVisitors=0,returningVisits=0,visitors=0,sessions=0;
  if(all){
    const keys=analyticsKeys('all');
    const [summaryRaw,pageRaw,menuRaw,deviceRaw,featureRaw,hourRaw,v,s]=await Promise.all([
  ...
  return{
    ...
    averageDailyVisitors:daily.length?Math.round(daily.reduce((sum,row)=>sum+row.visitors,0)/daily.length):0,
