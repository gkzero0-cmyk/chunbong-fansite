...
function analyticsKeys(day){
  ...
  return{
    ...
    searchQueries:`${p}search-queries:v1:${day}`,searchZero:`${p}search-zero:v1:${day}`,searchClicks:`${p}search-clicks:v1:${day}`
  };
}
function validAnonId(value){return /^[A-Za-z0-9_-]{12,96}$/.test(String(value||''))}
function normalizeSearchTerm(value){
  const text=safeText(value,40).replace(/\s+/g,' ').trim().toLowerCase();
  if(text.length<2||/https?:\/\/|www\.|@/.test(text)||/\d{7,}/.test(text))return '';
  return text;
}
function validEventType(value){return['page_view','active_time','menu_click','navigation_timing','web_vital','search_query','search_result_click','game_start','game_finish','tarot_start','tarot_result','schedule_open','data_tab_open','feedback_open','feedback_submit'].includes(String(value||''))}
async function ingestAnalytics(body){
  ...
  for(const event of events){
    ...
    if(type==='page_view'){
      ...
      const term=normalizeSearchTerm(event.target);if(!term)continue;
      const resultCount=Math.max(0,Math.min(500,Math.round(Number(event.resultCount)||0)));
      commands.push(['HINCRBY',keys.searchQueries,term,1],['HINCRBY',allKeys.searchQueries,term,1]);
      if(resultCount===0)commands.push(['HINCRBY',keys.searchZero,term,1],['HINCRBY',allKeys.searchZero,term,1]);
    }else if(type==='search_result_click'){
      const term=normalizeSearchTerm(event.target);if(!term)continue;
      const kind=safeText(event.resultKind,40).replace(/[\t\n\r]/g,' ')||'결과';
      const label=safeText(event.resultLabel,80).replace(/[\t\n\r]/g,' ')||'콘텐츠';
      const key=term+'\t'+kind+'\t'+label;
      commands.push(['HINCRBY',keys.searchClicks,key,1],['HINCRBY',allKeys.searchClicks,key,1]);
    }else{
...
async function analyticsOverview(days=7){
  ...
  const pages={},pageTime={},menus={},devices={},features={},hours={},perfTotal={},perfCount={},vitalTotal={},vitalCount={},vitalGood={},vitalNeeds={},vitalPoor={},searchQueries={},searchZero={},searchClicks={};
  let pageviews=0,activeMs=0,newVisitors=0,returningVisits=0,visitors=0,sessions=0;
  if(all){
    const keys=analyticsKeys('all');
    const [summaryRaw,pageRaw,pageTimeRaw,menuRaw,deviceRaw,featureRaw,hourRaw,perfTotalRaw,perfCountRaw,vitalTotalRaw,vitalCountRaw,vitalGoodRaw,vitalNeedsRaw,vitalPoorRaw,searchQueriesRaw,searchZeroRaw,searchClicksRaw,v,s]=await Promise.all([
      ...
      redisCommand('HGETALL',keys.searchQueries),redisCommand('HGETALL',keys.searchZero),redisCommand('HGETALL',keys.searchClicks),
    ...
    sumHash(pages,pageRaw);sumHash(pageTime,pageTimeRaw);sumHash(menus,menuRaw);sumHash(devices,deviceRaw);sumHash(features,featureRaw);sumHash(hours,hourRaw);sumHash(perfTotal,perfTotalRaw);sumHash(perfCount,perfCountRaw);sumHash(vitalTotal,vitalTotalRaw);sumHash(vitalCount,vitalCountRaw);sumHash(vitalGood,vitalGoodRaw);sumHash(vitalNeeds,vitalNeedsRaw);sumHash(vitalPoor,vitalPoorRaw);sumHash(searchQueries,searchQueriesRaw);sumHash(searchZero,searchZeroRaw);sumHash(searchClicks,searchClicksRaw);
  }else{
    for(const day of aggregateDates){
      const keys=analyticsKeys(day);
      const [summaryRaw,pageRaw,pageTimeRaw,menuRaw,deviceRaw,featureRaw,hourRaw,perfTotalRaw,perfCountRaw,vitalTotalRaw,vitalCountRaw,vitalGoodRaw,vitalNeedsRaw,vitalPoorRaw,searchQueriesRaw,searchZeroRaw,searchClicksRaw]=await Promise.all([
        redisCommand('HGETALL',keys.summary),redisCommand('HGETALL',keys.pages),redisCommand('HGETALL',keys.pageTime),redisCommand('HGETALL',keys.menus),
        redisCommand('HGETALL',keys.devices),redisCommand('HGETALL',keys.features),redisCommand('HGETALL',keys.hours),redisCommand('HGETALL',keys.perfTotal),redisCommand('HGETALL',keys.perfCount),
        redisCommand('HGETALL',keys.vitalTotal),redisCommand('HGETALL',keys.vitalCount),redisCommand('HGETALL',keys.vitalGood),redisCommand('HGETALL',keys.vitalNeeds),redisCommand('HGETALL',keys.vitalPoor),
        redisCommand('HGETALL',keys.searchQueries),redisCommand('HGETALL',keys.searchZero),redisCommand('HGETALL',keys.searchClicks)
  ...
  const searchTotal=Object.values(searchQueries).reduce((sum,value)=>sum+(Number(value)||0),0);
  const searchZeroTotal=Object.values(searchZero).reduce((sum,value)=>sum+(Number(value)||0),0);
  const searchClickTotal=Object.values(searchClicks).reduce((sum,value)=>sum+(Number(value)||0),0);
  const search={
    total:searchTotal,
    zeroTotal:searchZeroTotal,
    clickTotal:searchClickTotal,
    clickThroughPct:searchTotal?Math.min(100,Math.round(searchClickTotal/searchTotal*100)):0,
    topQueries:topRows(searchQueries,20).map(row=>({...row,zeroCount:Number(searchZero[row.key])||0})),
    zeroQueries:topRows(searchZero,20),
    topClicks:topRows(searchClicks,20).map(row=>{const [query,kind,label]=String(row.key).split('\t');return{query:query||'',kind:kind||'결과',label:label||'콘텐츠',value:row.value}})
  };
  return{
...
async function endpointHealth(base,label,path){
  ...
  try{
    ...
    const version=path==='/api/version'&&response.ok?await response.json().catch(()=>null):null;
    return{label,path,ok:response.ok,status:response.status,ms:Date.now()-started,...(version?{version}:{})};
  }catch(error){
...
async function githubProjectState(){
  ...
  try{
    ...
    const [repoResponse,commitResponse,recentResponse]=await Promise.all([
      ...
      fetch('https://api.github.com/repos/gkzero0-cmyk/chunbong-fansite/commits?sha=main&per_page=10',{headers,signal:controller.signal})
    ]);
    ...
    const recentCommits=(Array.isArray(recentPayload)?recentPayload:[]).slice(0,10).map(row=>({
      sha:String(row?.sha||''),
      shortSha:String(row?.sha||'').slice(0,7),
      message:safeText(String(row?.commit?.message||'').split(/\r?\n/,1)[0],180),
      author:safeText(row?.author?.login||row?.commit?.author?.name||row?.commit?.committer?.name||'',80),
    ...
    const result={available:Boolean(repo||commit),mainSha,repoSizeKb:Number(repo?.size)||0,updatedAt:repo?.updated_at||null,vercelStatus,recentCommits,cachedAt:Date.now()};
    if(hasRedis())try{await redisCommand('SET',GITHUB_STATE_CACHE_KEY,JSON.stringify(result),'EX',180)}catch{}
    return result;
  }catch{return{available:false,mainSha:'',repoSizeKb:0,updatedAt:null,vercelStatus:null,recentCommits:[],cachedAt:Date.now()}}finally{clearTimeout(timer)}
}

async function changelogStatus(base){
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),3500);
  try{
    const response=await fetch(base+'/api/content?type=changelog-history&summary=1',{headers:{Accept:'application/json','User-Agent':'chunbong-fansite-operator-health'},cache:'no-store',signal:controller.signal});
    if(!response.ok)return{ok:false,status:response.status,latest:null,generatedAt:null};
    const payload=await response.json(),latest=payload?.latest||null;
    return{ok:true,status:response.status,latest:latest?{sha:String(latest.sha||''),shortSha:String(latest.shortSha||latest.sha||'').slice(0,7),date:String(latest.date||''),time:latest.time||null,title:safeText(latest.title||latest.rawTitle||'',180),author:safeText(latest.author||'',80),url:String(latest.url||'')}:null,generatedAt:payload?.generatedAt||null};
  }catch(error){
    return{ok:false,status:0,latest:null,generatedAt:null,error:safeText(error?.name||error?.message||'request_failed',48)};
  }finally{clearTimeout(timer)}
}
...
async function operatorSystemStatus(req){
  ...
  const [github,today,storageStats,endpoints,changelog]=await Promise.all([
    githubProjectState(),
    hasRedis()?analyticsOverview(1).catch(()=>null):null,
    redisStorageStats(),
    publicEndpointHealth(base),
    changelogStatus(base)
  ]);
  const versionState=endpoints.find(row=>row.path==='/api/version')?.version||{};
  const mainSha=String(versionState.mainSha||github.mainSha||'');
  const exactSynced=typeof versionState.exactSynced==='boolean'?versionState.exactSynced:(github.available&&mainSha&&deploymentSha?mainSha===deploymentSha:null);
  const synced=typeof versionState.runtimeSynced==='boolean'?versionState.runtimeSynced:typeof versionState.synced==='boolean'?versionState.synced:exactSynced;
  const internalOnlyGap=versionState.internalOnlyGap===true;
  ...
  const healthIssues=[];
  if(synced===false)healthIssues.push('Production의 실제 사이트 코드가 GitHub main보다 뒤처져 있습니다.');
  ...
  if(!changelog.ok)healthIssues.push('업데이트 일지 자동 기록 상태를 확인해야 합니다.');
  ...
  return{
    checkedAt:now,
    deployment:{environment:String(process.env.VERCEL_ENV||process.env.NODE_ENV||'unknown'),sha:deploymentSha,url:deploymentUrl,mainSha,synced,runtimeSynced:synced,exactSynced,internalOnlyGap,pendingFileCount:Number(versionState.pendingFileCount)||0,pendingCommitCount:Number(versionState.pendingCommitCount)||0,compareStatus:String(versionState.compareStatus||''),vercel,rateLimited,rateLimitAt,retryAfter},
    ...
    repository:{available:github.available,sizeKb:github.repoSizeKb,updatedAt:github.updatedAt,recentCommits:Array.isArray(github.recentCommits)?github.recentCommits:[]},
    changelog,
