...
function buildSoopAnalytics(sessions = [], snapshots = [], live = {}, now = new Date(), options = {}) {
  ...
  result.recentSessions = (Array.isArray(result.recentSessions) ? result.recentSessions : []).map(raw => {
    const row = normalizeSessionViewerStats(raw);
    const date = sessionDate(row);
    const follower = dailySnapshotMeta(points, date, 'followerCount');
    const fanclub = dailySnapshotMeta(points, date, 'fanclubCount');
    return {
      ...row,
      date: row.date || date,
      followerCount: follower.count ?? finite(row.followerEnd),
      followerDelta: follower.delta ?? row.followerDelta ?? null,
      fanclubCount: fanclub.count ?? finite(row.fanclubEnd),
      fanclubDelta: fanclub.delta ?? row.fanclubDelta ?? null
    };
  });
