...
async function fetchChunbongData(deps = {}) {
  ...
  const [vodResult, clipsResult, liveResult, youtubeResult, channelResult] = await Promise.allSettled([
    fetchVod(), fetchClips(), fetchLive(), fetchYoutube(), fetchYoutubeChannel()
  ]);
  ...
  const live = liveResult.status === 'fulfilled' ? liveResult.value : { live: null, title: '', startedAt: '', viewerCount: null, source: `https://play.sooplive.com/${SOOP_ID}` };
  ...
  const successfulSoop = [vodResult, clipsResult, liveResult].some(result => result.status === 'fulfilled');
  ...
  return {
    ...
    sources: {
      soop: `https://www.sooplive.com/station/${SOOP_ID}`,
