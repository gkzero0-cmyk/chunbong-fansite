const { CAFE_ID, naverHeaders, first, getJson } = require('./_shared');
...
async function handler(req, res) {
  ...
  try {
    const item = await fetchFanartDetail(req.query?.id);
    return res.status(200).json({ item, fallback: !item.images.length });
  } catch (error) {
    return res.status(200).json({ item: null, fallback: true, reason: error.message });
  }
}

module.exports = handler;
module.exports.extractImages = extractImages;
module.exports.fetchFanartDetail = fetchFanartDetail;
