(()=>{
  ...
  const tarotTopicLabel=row=>TAROT_TOPIC_LABELS[String(row?.topic||'general')]||String(row?.topic||'종합타로');
  const tarotModeLabel=row=>String(row?.mode||'detail')==='quick'?'빠른 타로':'상세 타로';
  const tarotRecordTitle=row=>tarotTopicLabel(row)+' · '+tarotModeLabel(row);
  ...
  function recordTarot(entry={}){
    ...
    const row={
      ...
      topic:String(entry.topic||'general'),mode:entry.mode==='quick'?'quick':'detail',spreadId:String(entry.spreadId||'single'),
  ...
  async function setAlertEnabled(enabled){
    ...
    void syncPushSubscription(true,state).then(pushEnabled=>{
      const latest=read();
      if(!latest.alerts.enabled)return;
      latest.alerts.pushEnabled=Boolean(pushEnabled);write(latest);
    });
    return true;
  }
  ...
  function alertCopy(state){
    if(!state.alerts.enabled){
      if(!('Notification'in window))return{label:'이 브라우저에서는 알림을 지원하지 않습니다.',body:'현재 브라우저에서는 방송 알림을 사용할 수 없습니다. 알림을 지원하는 최신 브라우저에서 다시 시도해 주세요.'};
      if(Notification.permission==='denied')return{label:'브라우저 알림 권한 차단됨 · 설정에서 허용 필요',body:'브라우저에서 이 사이트의 알림 권한이 차단되어 있습니다. 주소창의 사이트 설정에서 알림을 허용한 뒤 다시 ON을 눌러 주세요.'};
      return{label:'기본 설정 OFF · 직접 켠 경우에만 알림',body:'알림은 현재 꺼져 있습니다. ON으로 바꾸면 브라우저가 알림 권한을 요청합니다.'};
    }
    if(state.alerts.pushEnabled)return{label:'백그라운드 Push ON · 앱을 닫아도 알림',body:'알림이 켜져 있습니다. 원하는 방송 종류와 미리 알림 시간을 선택하세요.'};
    return{label:'알림 ON · 백그라운드 Push 연결 중/확인 필요',body:'알림은 켜졌습니다. 백그라운드 Push 연결을 확인 중이며, 연결되지 않으면 이 탭이 열려 있을 때 알림을 확인합니다.'};
  }
  ...
  function renderDashboard(){
    ...
    const state=read(),game=gameSnapshot(),challenge=dailyChallenge(),recent=state.recent,latestTarot=state.tarot[0],earned=game.achievements.filter(x=>x.earned),collections=Object.entries(COLLECTIONS),counts=Object.fromEntries(collections.map(([key])=>[key,state.favorites.filter(item=>(item.collection||'later')===key).length])),tarotRows=state.tarot.slice().sort((a,b)=>Number(Boolean(b.pinned))-Number(Boolean(a.pinned))||String(b.createdAt||'').localeCompare(String(a.createdAt||''))),alert=alertCopy(state);
    ...
    root.querySelector('[data-personal-alert-toggle]')?.addEventListener('click',async e=>{
      const button=e.currentTarget,label=button.querySelector('span'),next=!read().alerts.enabled;
    ...
    root.querySelectorAll('[data-view-tarot]').forEach(button=>button.addEventListener('click',()=>{
      ...
      dialog.innerHTML='<div class="personal-tarot-archive-inner"><header><div><small>TAROT ARCHIVE</small><h2>'+esc(tarotRecordTitle(row))+'</h2><p>'+esc(formatDate(row.createdAt))+'</p></div><button type="button" data-close-archive aria-label="타로 기록 닫기">×</button></header>'+
        '<section class="personal-tarot-archive-meta"><h3>리딩 정보</h3><p><b>주제</b> · '+esc(tarotTopicLabel(row))+'</p><p><b>방식</b> · '+esc(tarotModeLabel(row))+'</p>'+(row.question?'<p><b>질문</b> · '+esc(row.question)+'</p>':'')+'</section>'+
